...
**2026-09-23 用户实测修正正在验证：**针对 build191 的整行展开、快捷键持久化、
浏览器/PDF 取词、无选区快速输入、权限本地化和多窗口导航问题进行修正。
主界面改为真正的单窗口切页，视觉改为中性底色、原生材质和少量功能色。
新增真实原生控件连续操作与固定种子随机交互检查；外部浏览器与权限实机覆盖单独说明。
状态见[本轮实施记录](MACOS_TODO.md#native-usability-repair)；本轮新包尚未交付，不能以旧包通过替代。

**前置 Pearl 原生界面检查点：**上一轮的暖白/紫色方案统一翻译、快速结果、历史、词典、
设置、截图状态与关于界面，保留原生编辑器和现有工作流程；不用宣传文案或 Demo 模拟数据。
新增词典页面仅离线查询，不因未命中而调用 AI。推荐包为 **0.1.0/build191**，源码 `1f8d9ed`；
...
已加入系统管理的“登录时启动”及批准/关闭/刷新入口，本包另有独立Mac版本0.1.0/构建191；
Pearl 暖白/深紫主题已用于主要界面，侧栏在窄窗口收为图标导航；没有宣传口号。
...
producer执行983个原生测试（960通过、23构包前可选skip、零失败）并保留194张PNG，
...
- 源码：`1f8d9ed8361572517abb09a5098622b1cc5a476f`；版本0.1.0、构建191；
  [run35693532432](https://github.com/mclight-ship-it/cc-translate/actions/runs/35693532432)；
  [完整App下载](https://github.com/mclight-ship-it/cc-translate/actions/runs/35693532432/artifacts/10680028105)；
  [原生离屏截图](https://github.com/mclight-ship-it/cc-translate/actions/runs/35693532432/artifacts/10680380825)。
- 内层`CCTranslateMac-P0.zip`：22,520,928 bytes；
...
**本轮优先复测：**退出旧版后解压替换 App；关于窗口应显示0.1.0/build191，配置/CLI/账号保持原样。
检查暖白/深紫主题和侧栏切换；本地词典查询不应覆盖翻译草稿，未命中不应自动触发 AI；
