...
**翻译提速已实现并通过工程验收：**当前包 **0.1.0/build206**，源码 `8007747`；
见[性能清单与实测](MACOS_TODO.md#native-translation-latency)和[下载](#native-translation-user-check)。
Codex 版本检查缓存、单进程预热、Claude 单槽空闲进程、首段立即刷新和词典预检已接通。
1060 项原生测试（23 可选 skip、零失败），同包三系统各 260 进程/851 核心/21 Foundation 通过。
官方 Codex 无模型冷准备 744–770 ms、驻留复用 0.29–0.47 ms；不等于真实翻译倍数。
补充的旧新版同机 60 请求对照：固定合成 Codex 响应下，连续请求含准备的完成中位数
约 492/522 ms 降至 2.99/3.07 ms；完全冷请求约 2.7 秒，未见提升。
这是客户端请求链路开销，不是实际模型完成耗时；方法、原始数据与限制见上述性能记录。
...
已加入系统管理的“登录时启动”及批准/关闭/刷新入口，本包另有独立Mac版本0.1.0/构建206；
...
本轮增加 Codex 版本检查缓存和两服务的轻量预热；明确翻译意图下准备当前服务，闲置十分钟回收。
首段流式文字立即显示，普通句子不再先做必定无结果的词典请求；短多词词条仍优先查词。
“诊断 → 翻译耗时”仅在内存保留二十次数字记录，不含原文、译文或账号。
producer执行1060个原生测试（1037通过、23个需设置App路径的可选skip、零失败）并保留原生PNG，
...
- 源码：`8007747d9e01e451c3e53978d026bab287ba820f`；版本0.1.0、构建206；
  [run35961227537](https://github.com/mclight-ship-it/cc-translate/actions/runs/35961227537)；
  [完整App下载](https://github.com/mclight-ship-it/cc-translate/actions/runs/35961227537/artifacts/10793790676)；
  [原生截图与流程记录](https://github.com/mclight-ship-it/cc-translate/actions/runs/35961227537/artifacts/10793092959)。
- 内层`CCTranslateMac-P0.zip`：22,611,028 bytes；
  SHA-256 `21976b007e33ca1eae995867daf4acab3547ccc504afe5442851c3efe5b9f9dc`。
...
**本轮优先复测：**退出旧版后解压替换 App；关于页面应显示0.1.0/build206，配置/CLI/账号保持原样。
观察连续快速翻译和闲置后首个请求；诊断耗时可区分取词、准备和首段显示，
但真实模型网络速度仍取决于同模型、同文本和同缓存状态的对照。
