...
已加入系统管理的“登录时启动”及批准/关闭/刷新入口，本包另有独立Mac版本0.1.0/构建153；
...
producer执行877个原生测试（854通过、23构包前可选skip、零失败）并保留151张PNG，
...
- 源码：`8495d676661128aaf59bb2cc1f0ea68daa65d0da`；版本0.1.0、构建153；
  [run35453455630](https://github.com/mclight-ship-it/cc-translate/actions/runs/35453455630)；
  [完整App下载](https://github.com/mclight-ship-it/cc-translate/actions/runs/35453455630/artifacts/10587907156)；
  [原生离屏截图](https://github.com/mclight-ship-it/cc-translate/actions/runs/35453455630/artifacts/10587716895)。
- 内层`CCTranslateMac-P0.zip`：20,671,298 bytes；
  SHA-256 `d7a66c0ba6a736e96f6e6019445cca3499e0992329f51ce7b0afd2b2583c81cd`。
  artifact保留到2026-09-26T16:23:04Z；过期时只取新的经核验固定run，不使用未知镜像。
...
build160已实际通过六个临时签名安装/拒绝/取消场景、890原生项及macOS15/14链路；
macOS26的一个旧空RTF测试在fixture发布尚未反映到AppKit计数时开始读取，导致两个断言失败。
完整run未通过，当前修正测试发布就绪等待，保留生产读取期间换主拒绝。
这不是用户使用前置，不需要调整CLI/账号，也不将未通过整体验收的build160替换为推荐包。
...
更新fixture另加入空闲CPU/内存采样，等待自身Mac执行：在一次性App副本中，更新检查前静置5秒，
