...
**当前可下载检查点：原生界面、图片/OCR翻译、词典来源、复制隔离、字号/摘要/历史条数/输入上限、结果位置、双击复制间隔及截图全局快捷键；移植尚未完成。**
最新批次3ffb141已通过[完整App与同包三系统验收](MACOS_TODO.md#native-capture-shortcut-checkpoint)。
下文保留更早失败与修复过程；恢复默认设置仍在后续源码中验证，不包含在本包内。
...
当前推荐下载3ffb141保留bf89495已验图片链路，并已完成完整App同包三系统验收，见
[当前批次](MACOS_TODO.md#native-capture-shortcut-checkpoint)，未声称完成全部移植。
以下保留早期工程包记录：
...
9b55942已实际通过原生11项位置测试、125PNG与完整同包三系统验收；当前3ffb141下载继续包含该功能。
...
实际watch exit0，完整包字节已独立核验。结果位置继续包含在当前3ffb141下载中。
...
对已提交的翻译、结果和历史没有影响。第二次按键后仍只等待0.5秒的新复制内容，不扩大剪贴板有效期。
...
完整App及同包15/14/26已独立核验，当前下载已升级为3ffb141，包含此快捷键；具体制品与hash见
...
本节描述新源码，当前修正仍待独立Mac复验、完整包与同包多系统验证；当前推荐包不包含此功能。
91773dd已编译，13新方法中11通过；首轮发现正常停止遗漏恢复状态清理，以及测试按钮OCR定位失败。
正常停止现与transport failure一样退休待处理恢复请求；测试复用唯一原生按钮定位。
修正8f898aa / run35341468695已正常通过1850项hook，正在Mac复验；917已实际watch exit1，
...
零失败），英中历史流程完整通过；后续完整App及同包14与26也已通过；当前下载已进一步升级为3ffb141。
...
当前新增明确图片发送、稳定词典来源按钮、关联复制回退、剪贴板主线程隔离，
以及字号、长文摘要、历史保留条数、输入上限、结果窗口位置、双击复制间隔和截图全局快捷键设置；保留明确刷新模型目录、设置/主窗口/截图共享选择、
主动纯文本粘贴、
...
producer执行798个原生测试（775通过、23构包前可选skip、零失败）并保留131张PNG，
同一个App在15.7.9/14.8.9/26.6.2各通过232进程/665核心/21后置Foundation及独立About1；
...
- 源码：`3ffb1419cba23d65aa151660dbba419be4496354`；
  [run35338542005](https://github.com/mclight-ship-it/cc-translate/actions/runs/35338542005)；
  [完整App下载](https://github.com/mclight-ship-it/cc-translate/actions/runs/35338542005/artifacts/10545236397)；
  [原生离屏截图](https://github.com/mclight-ship-it/cc-translate/actions/runs/35338542005/artifacts/10544129081)。
- 内层`CCTranslateMac-P0.zip`：19,561,280 bytes；
  SHA-256 `e1b9a82244323f1474711cdd874185f4b7e0f7f433e9e3a8600a6187cd8c7af8`。
  artifact保留到2026-09-25T11:40:13Z；过期时只取新的经核验固定run，不使用未知镜像。
...
- Apple Silicon、macOS14+候选；Intel未支持承诺。用户不需要Xcode/Python/Git/付费开发者账号。
  ...
  结果位置、双击间隔与截图全局热键开关已包含；恢复默认、Claude等仍在继续。
...
5. History可搜索全部已保存记录的原文、译文或日期，按文字/词典/代码/OCR筛选，
   ...
   可选点“刷新模型”读取当前CLI的列表；之后主窗口和截图预览也可选择，空/失败仍可手填ID。
