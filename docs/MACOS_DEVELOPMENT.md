...
**当前推荐开发包：0.1.0/build223**，源码 `1112a6c`；
见[本轮优化与证据](MACOS_TODO.md#native-translation-observability)和[在线下载](#native-translation-user-check)。
新增有界短时内存结果复用、CLI解析模型信息、首个可读内容/摘要/截图阶段计时及摘要阅读位置保护。
原生1105项（23可选skip、零失败），同包macOS14/15/26各260进程/889核心/21Foundation通过。
关闭历史的10次相同合成请求从10次provider调用降为1次调用/9次命中；
不是新内容或真实模型的速度百分比。真实GPT对照因当前自定义服务/外部认证路径不能确认订阅计费而未执行，
已提供默认零调用、显式订阅检查的评估工具；生产模型、prompt和摘要优先顺序不变。

**上一轮提前预热验收：**包 **0.1.0/build219**，源码 `252e07f`；
见[本轮实施与验证](MACOS_TODO.md#native-translation-preparation)和[下载](#native-translation-user-check)。
已启用快捷键时首次有效 Cmd+C、截图开始、休眠唤醒及快捷键/权限恢复可提前准备已配置服务；
首次复制不读取正文，截图取消不提交内容，唤醒不重复创建助手，前台翻译始终优先。
1075 项原生测试（23 可选 skip、零失败），新增 11 项应用预热回归全部通过；
同包三系统各 260 进程/851 核心/21 Foundation 通过。
官方 Codex 无模型冷准备 779–1087 ms、驻留复用 0.43–0.61 ms；只是本轮准备探针，
不是新旧版真实 GPT 端到端 A/B，不能由此给出翻译提速百分比。
模型、prompt、摘要优先顺序和现有 CLI 登录/计费路径不变，不切换到 Apple 翻译。

保留[第一轮性能优化](MACOS_TODO.md#native-translation-latency)：
Codex 版本检查缓存、单进程预热、Claude 单槽空闲进程、首段立即刷新和词典预检已接通。
此前 build202/build206 同机 60 请求对照：固定合成 Codex 响应下，连续请求含准备的完成中位数
约 492/522 ms 降至 2.99/3.07 ms；完全冷请求约 2.7 秒，未见提升。
这是旧轮客户端请求链路开销，不是 build219 实际模型完成耗时；方法、原始数据与限制见上述性能记录。
预热只初始化当前选定服务，不发送原文、不启动模型生成；设置/历史/词典页面本身不授权预热。
打开翻译窗口、已配置服务后的快速输入窗口，或已启用且成功恢复选区快捷键的启动，可以提前准备助手；
默认关闭快捷键或尚未配置服务的启动仍不启动翻译引擎。首次使用的配置读取仍保持独立，
配置助手不会仅为预热升级为翻译助手。用户提交始终优先于后台准备，不自动重放失败请求。
计时位于诊断窗口，内存中最多保留二十次请求，应用退出即清空；
首字指标指界面数据首次更新，不是实际屏幕绘制或同条件真实模型性能结论。

**2026-09-24 用户实测修正版已通过工程验收：**针对 build191 的整行展开、快捷键持久化、
浏览器/PDF 取词、无选区快速输入、权限本地化和多窗口导航问题完成修正。
主界面改为真正的单窗口切页，视觉改为中性底色、原生材质和少量功能色。
新增真实原生控件连续操作与固定种子随机交互检查；外部浏览器与权限实机覆盖单独说明。
该轮包 **0.1.0/build202**，源码 `7953587`；1037 项原生测试（23 可选 skip、零失败），
七个连续流程及三组共 180 步随机交互通过；同包三系统、六种临时签名升级场景和独立包核验通过。
状态与边界见[本轮实施记录](MACOS_TODO.md#native-usability-repair)，下载见[当前开发包](#native-translation-user-check)。

**前置 Pearl 原生界面检查点：**上一轮的暖白/紫色方案统一翻译、快速结果、历史、词典、
设置、截图状态与关于界面，保留原生编辑器和现有工作流程；不用宣传文案或 Demo 模拟数据。
新增词典页面仅离线查询，不因未命中而调用 AI。当时包为 **0.1.0/build191**，源码 `1f8d9ed`；
983 原生测试（23 构包前可选 skip，零失败）、194 PNG、完整同包三系统及独立包审计通过。
证据见[Pearl 实施记录](MACOS_TODO.md#native-pearl-ui)，下载见[当前开发包](#native-translation-user-check)。

**前置 build183 反馈修正（新包保留）：**Dock 改为仅有窗口时显示；截图选区后直接翻译，文字/图片模式放到设置；
尝试修正浏览器/PDF 双击复制兼容、权限设置跳转，恢复作者赞助并简化菜单。
其中浏览器/PDF 取词仍被后续用户实测报告为失败，继续按本轮反馈处理。
该检查点为 **0.1.0/build183**，源码 `2fa45ee`；961 原生测试（23 构包前可选 skip，零失败）、
173 PNG、完整同包三系统及独立包审计通过，见[本轮清单](MACOS_TODO.md#native-quick-translation-feedback)。

**前置 build174 体验调整：**补齐窗口找回，并将设置改为
“翻译 / 快捷键 / 外观 / 更多”，技术详情按需展开。该检查点为 **0.1.0/build174**，
...
产品主路径仍是快速取词、快捷翻译和小结果浮窗；输入/结果双栏窗口保留为按需打开的辅助入口，
不因新增工作窗口而把常规使用变成复杂的翻译工作台。
设置优先呈现标签、当前值和操作。自定义模型、安装位置和技术信息按需展开；
必要的权限、失败恢复及删除后果必须保留，不能用隐藏错误或放宽后台校验换取界面简洁。
...
build223另有独立内存cache：最多20项、合计512KiB的UTF-8输出及哈希key、300秒TTL，
...
- 双击 Cmd+C 只观察用户事件，不吞正常复制，不写哨兵，不模拟复制，不撤销主动复制结果。
  AX-only 诊断不读剪贴板。产品路径用事件派发前的 changeCount、前台进程及启动时间、
  两次按键与期间的取消事件关联本次复制，不要求网页/PDF提供稳定的AX焦点对象。
  不读取手势前未变化的内容；重复复制同一文本不要求文本内容不同。
- 本次复制最多等待2秒发布；已发布的延迟数据给独立读取进程最多5秒（从第二次按键起），
  成功响应还允许0.5秒进程清理。剪贴板改版会重试，但不延长这次手势的总期限。
  按顺序尝试同一项的文本表示及RTF，多个文本项以换行连接；文件/PDF/浏览器元数据
  只忽略而不作为文字来源，隐藏内容标记仍拒绝。最终仍受8192 UTF-8字节协议预算限制。
- 无选区、空白选区或未观察到本次复制请求快速输入，不制造翻译失败、不提交旧剪贴板。
  确有复制但格式不支持、超限、读取失败等仍与无选区区分。
- `nativeSelectionShortcutEnabled`保存用户选择，`monitorEnabled`只表示运行状态。
  启动时在已有AX与输入监控授权、未启用Secure Input时自动恢复；短暂停用和退出不清除选择，
  后台以只读权限检查重试，绝不因此弹授权提示。用户关闭或恢复默认设置才持久化关闭。
...
当前正常入口采用[快捷翻译流程](MACOS_TODO.md#native-quick-translation-feedback)：
设置“快捷键 → 截图翻译”默认为“识别文字”，选区后本地 OCR 完成即提交文字翻译，
不再打开编辑/确认页。等待或失败使用可缩小的状态窗口；无文字时可以重新截图或关闭，
不会自动改发图片。新翻译意图、取消和关闭会阻止旧识别结果提交或重新弹窗。
选择“发送图片”时走下节的直接图片路径，不启动本地 OCR。

以下保留早期手动预览流程的工程合同，已由082aad6同App三系统自动化验证；
完整源码/制品证据见[截图检查点](MACOS_TODO.md#native-capture-checkpoint)。
手动编辑/确认界面仍保留于代码及回归测试，不再是正常截图入口的必经步骤。
...
当前在设置“快捷键 → 截图翻译”选择“发送图片”，之后每次截图选区完成就直接发送，
不需要 OCR 或第二次确认；这是图片请求，不是把本地 OCR 文字改名为图片翻译。
默认仍是“识别文字”；模式在此 Mac 保留，恢复默认设置成功后回到文字模式。
最新包见[下载与操作](#native-translation-user-check)，未声称完成全部移植。
前置f9781b7保留bf89495图片链路并补齐Claude图片服务，已完成完整App同包三系统验收，
见[早期批次](MACOS_TODO.md#native-restore-defaults-checkpoint)。以下保留早期工程包记录：
已提供的bfd9382工程验证包不受后续测试诊断替换；667d17b只增加私有剪贴板元数据诊断，
实际原生测试失败，未发布新App。其独立图片/随包阶段通过不是整体CI通过，
也不要求用户重新安装CLI、刷新目录或重做已通过的普通翻译。

1. 在设置中选择“发送图片”，确认当前翻译服务和模型；该选择会用于后续截图。
2. 从主窗口、菜单栏或已启用的全局截图快捷键开始截图，完成选区后直接翻译。
   不要求先刷新模型目录，不增加精确CLI版本或模型白名单；图片路径不等待 OCR。
3. 图片操作只发送已选区域，沿用所选Codex `localImage` 或Claude图片请求；
   不发送整个显示器帧，不自动改发OCR文字，不自动重试或切换模型。
4. 结果显示在普通结果窗口；成功提交后释放截图预览和保留帧。
   可以取消自己发起的翻译，不取消后来开始的其他翻译。
   图片结果可复制输出和执行输出相关操作；不会伪造原文或提供虚假的双语复制。

只有已选择图片模式并完成选区，才编码选区并创建私有PNG；目录/文件权限分别为0700/0600。
截图可能包含敏感内容，图片模式会直接交给当前服务处理，可能计费；不希望上传图片时使用文字模式。
...
[当前可用性检查点](MACOS_TODO.md#native-result-copy-usability)。
首轮真实Mac已通过复制与窗口缩放回归，但发现紧凑短结果被状态区占位挤出视口；
后续2392c14已改为内容自然高度、长提示才滚动，并提升错误文字可读性；
874原生项与同包三系统均通过；后续8495d67又补齐关闭后错误提示和窄窗口按钮，
877原生项及同包三系统均通过，当前推荐build153。原断言及失败图片保留。
...
先按下再松开，进入与菜单相同的截图流程；按设置中的“识别文字 / 发送图片”直接翻译，
不再经过 OCR 预览确认。文字模式仅发送本地识别结果，图片模式直接发送选区图片。
...
已加入系统管理的“登录时启动”及批准/关闭/刷新入口，本包另有独立Mac版本0.1.0/构建223；
采用中性浅/深底色、原生侧栏材质与少量功能色，不再铺紫色；没有宣传口号。
翻译、历史、词典、设置和关于在同一个主窗口内切换，保留草稿、原生编辑器、撤销与滚动位置。
无选区时打开独立轻量快速输入；读不到选区时提供粘贴入口，不翻译旧剪贴板。
词典页提供真实离线查词和下载管理，不自动回退到 AI；快速浮窗新增置顶及打开主窗口操作。
Dock / Cmd-Tab 仅在有产品窗口打开时出现；隐藏或最小化仍可找回原窗口，
关闭最后一个窗口后 Dock 消失，菜单栏继续运行。设置分为“翻译 / 快捷键 / 外观 / 更多”，
自定义模型、安装位置及技术说明按需展开；缺少取词权限时直接进入“快捷键”。
展开行的文字和空白区域均可点击，也可通过键盘操作；权限状态中英一致，不显示内部诊断。
双击复制开关记住用户意图，重启或权限暂时不可用不自动取消用户偏好。
本轮修正结果浮窗过高/越界、纵向缩放及浏览器双击复制的正常文本兼容；
关闭浮窗后新一次取词失败仍会给出提示，窄窗口的复制/重新翻译按钮保留完整文字；
新增原生“检查更新”菜单/设置/独立窗口和Sparkle框架，无发布渠道时提供明确的手动下载入口；
新增卸载入口，默认保留数据，可明确选择只清理本应用的数据；排空时禁用迟到操作，不删除CLI/账号/Node；
...
保留原生关于/第三方许可及作者赞助入口；截图选区后按设置直接翻译文字或图片，
无需确认 OCR；保留全库历史搜索、本地词典下载/开关/删除/来源许可与六种结果动作。
保留 Codex 版本检查缓存和两服务的轻量预热；明确翻译意图下准备当前服务，闲置十分钟回收。
本轮将准备前移到首次有效复制、截图开始、休眠唤醒和监听恢复；不提前读取或发送用户正文，
不发空模型生成请求，不影响摘要优先顺序。休眠仅回收空闲助手，前台请求不被后台准备打断；
配置/权限不足、关闭/取消/禁用后的晚到回调不应产生内容提交或后台复活。
首段流式文字立即显示，普通句子不再先做必定无结果的词典请求；短多词词条仍优先查词。
新增与历史开关独立的短时精确内存复用；仅可确认同一Codex驻留进程及输出契约时命中，
显式重译、设置变更、清历史和退出均清理，OCR/图片/结果动作不走新cache。
摘要结果不再被后续译文自动推离视口；保留用户手动滚动与选区，普通结果仍跟随末尾。
“诊断 → 翻译耗时”仅在内存保留二十次阶段耗时与安全模型标识，不含原文、译文或账号；
区分首次可读内容、摘要边界、OCR/选区等待、图片准备与缓存，并对同组已完成样本列出n/median/P95。
producer执行1105个原生测试（1082通过、23个需设置App路径的可选skip、零失败）并保留原生PNG，
这些可选项在后续包内阶段另行通过（Foundation 21、About 1、词典到绘制 1）；
七个独立进程连续流程、三组固定种子各60步随机交互全部完成，另验证跨页关闭取消与快速结果后续请求归属。
同一个App在15.7.9/14.8.9/26.6.2各通过260进程/889核心/21后置Foundation及独立About1；
三个系统的生产Vision4、主动粘贴46、关联复制读取12和实际App只读worker12也实际通过。
完整App已独立字节核验；不将这些测试冒充全局快捷键或真实目标编辑器的人工验收。
旧包的用户翻译正向反馈不是新GUI/TCC、所有CLI版本或账号模型的完整验收。

- 源码：`1112a6c6e982935456ab8797dd155c7b83550cc6`；版本0.1.0、构建223；
  [run36144091850](https://github.com/mclight-ship-it/cc-translate/actions/runs/36144091850)；
  [完整App下载](https://github.com/mclight-ship-it/cc-translate/actions/runs/36144091850/artifacts/10869524888)；
  [原生测试记录](https://github.com/mclight-ship-it/cc-translate/actions/runs/36144091850/artifacts/10869936229)。
- 内层`CCTranslateMac-P0.zip`：22,662,666 bytes；
  SHA-256 `50e5010fdac1ca042a7c79096c647689c39146d9ebffa443a61707a5033448ca`。
  artifact保留7天；过期时取新的经核验固定run，不使用未知镜像。
...
**本轮优先复测：**退出旧版后解压替换 App；关于页面应显示0.1.0/build223，配置/CLI/账号保持原样。
重复普通文字翻译可看memory_cache_hit，但自定义服务、无法确认身份等配置可能不会命中；
诊断中缺失的CLI解析模型保持unknown，不要求用户为缓存更换账号或服务。
长文摘要出现后，后续译文增长不应把摘要顶走；选中文字时保留原有延后格式转换的行为。
观察首次复制后双击取词、截图翻译、休眠唤醒后的首个请求，以及关闭再启用快捷键后的翻译；
单次复制只准备而不翻译，取消截图不能晚到提交，前台翻译中休眠不应被预热逻辑主动取消。
诊断耗时可区分取词、准备和首段显示；本轮已证明提前调度与复用，没有真实 GPT 同机端到端 A/B，
不能把无模型准备数据视作翻译提速百分比，真实网络和推理耗时仍需同模型/文本/缓存状态的对照。
检查中性浅/深主题、原生侧栏材质与同一窗口切页；本地词典查询不应覆盖翻译草稿，未命中不应自动触发 AI；
浮窗置顶开关应切换层级，打开主窗口应保留未提交草稿；窄窗口仍可缩放、滚动及使用操作按钮。
启动但没有窗口时只显示菜单栏入口；打开窗口后出现 Dock 图标，也能用 Cmd-Tab 找回。
输入未提交草稿后隐藏或最小化窗口，点击 Dock 应找回原窗口和草稿；
关闭最后一个窗口后 Dock 应消失，之后通过菜单栏打开窗口，Dock 再出现。
设置默认进入“翻译”；切换四类设置不丢草稿，自定义模型和安装详情在需要时展开。
点击展开行的文字或空白处应能开合；双击复制开启后退出并重开，用户开关仍应保留。
权限不足时显示等待授权而非清除开关；中文页面不应显示 Accessibility granted 等内部状态。
菜单“设置”不再单独显示齿轮，翻译入口不再带 Cmd+N。
未授权时从菜单启用双击 Cmd+C，设置应直接打开“快捷键”，已有窗口也应切到此页。
在设置“快捷键 → 截图翻译”选择模式后截图，应直接得到结果，不再确认 OCR。
“关于”页面的“请作者喝杯咖啡”应打开原有赞助二维码。
快捷翻译仍是主路径，不要求使用双栏窗口；此前窗口/浏览器问题可顺手复测：
未选文字时双击复制应打开快速输入，输入或粘贴后显式翻译；取消后再开应为空，未关闭前重复唤起应保留草稿。
出现输入过长等错误时，检查结果窗是否仍在屏幕内，能否拖上下边/角缩小；
长结果应滚动而非撑高窗口，错误提示和底部操作应可见。
关闭结果窗后，不选文字再尝试取词应打开快速输入，而不是旧错误或旧翻译；把窗口缩窄时，
复制/双语/重新翻译文字应完整显示，必要时工具栏分两行，而不是截断按钮名称。
在此前失败的网页/PDF和其他可以普通Cmd+C复制的应用中选中文字，再双击Cmd+C，
应得到本次选中文字，不是手势前的旧剪贴板；单次复制不会触发翻译。
新实现解决第一次复制已完成后才记录基线、第二次复制被去重，以及延迟提供剪贴板数据的情况；
本轮进一步改为新剪贴板优先，支持 PDF 元数据与文本并存、富文本/多项文本和延迟数据，
不要求浏览器先返回 AX 选区对象；仍校验来源身份和复制版本。
读取在独立进程执行，不额外发送复制键、不清空或改写剪贴板。
这些真实目标应用行为仍需用户环境验证，不以私有pasteboard/注入事件测试代替。
菜单“检查更新…”仍显示未发布渠道说明，只有点击“查看已验证下载”才打开浏览器。
没有已发布feed，因此此包不执行真正的签名升级；无需你为此准备账号、密钥或额外CLI。
卸载基础和排空期间的交互边界已包含于本包；临时签名升级六场景亦独立验证，
不要求用户参与卸载测试，尤其不要为了验证而删除日常数据。
本包把随包Python整树从`Contents/Helpers/python`移到`Contents/Resources/python`：
标准库不是原生子bundle，不能放在系统预留的嵌套代码目录。Swift启动器、C桥定位、
包内测试及CI归档检查一起切换；不迁移用户配置、历史、词典或CLI路径。
此布局已通过本包自己的完整同包三系统、安装重启/坏签名/错密钥/两个取消/下载失败验证。
历史build160曾因macOS26空RTF测试过早读取而失败；build161只修正fixture的发布就绪等待，
保留生产换主拒绝及全部原断言，并已在三个系统通过。无需调整CLI/账号或新增使用前置。
...
5. History可搜索全部已保存记录的原文、译文或日期，按文字/词典/代码/OCR筛选，
   ...
   Codex可选点“刷新模型”读取列表；主窗口和截图翻译共享选择，空/失败仍可手填ID。
...
6. `Screenshot / 截图`或菜单栏`Screenshot translation… / 截图翻译…`打开区域截图工作流。
   默认“识别文字”：选区后自动本地识别，随后直接翻译文字，不上传图片、不再确认 OCR。
   本地识别本身不需要CLI或账号；后续AI翻译使用当前服务。无需先打开Diagnostics。
   可选在Settings开启截图快捷键，按下并松开⌘⌥⇧X进入相同流程；默认关闭，开启本身不申请截图权限。
   需要图片翻译时，在设置“快捷键 → 截图翻译”切换为“发送图片”；
   此后选区完成即发送图片，不运行 OCR。这是真正的模型图片请求，可能计费，
   图片可能包含敏感内容，请在开始截图前确认所选模式。
   文字模式下识别为空或失败时显示小型提示，可重新截图或关闭，不会自行改发图片。
...
5. 在设置中分别选择“识别文字”和“发送图片”，再用无敏感内容截图，确认选区后直接翻译。
   模型请求仍使用已有账号；选区完成即按已选模式发送，无第二次确认。
   继续开发不以完成此人工检查为前置。

**模型目录简短检查（可选）：**

1. Settings点击“刷新模型”，检查显示的模型名称和ID；取消/空/失败均应能继续使用原模型或手工ID。
2. 在设置中选模型后正常退出重开，选择应保持；主窗口与下一次截图翻译使用所选模型。
...
性能目标（固定样本已在build163和165同包三系统测量）：分类 P95 ≤2ms；常驻 IPC ≤5ms；词典查询+格式化 ≤10ms；
触发到完整词典首屏 ≤150ms。固定样本/冷热分开，LLM 首字与完成交错比较；
记录空闲 CPU、内存、长稳与休眠，不用单次均值替代 P95。
`tools/macos/performance.py`只在CI显式执行，使用随包隔离Python/Core和外部固定词库；
8类文本与5个词条分别记录首次、3次预热、100次样本，P95采用nearest rank。
常驻往返为现有config_load写入至completed解析，包括配置处理/JSON/队列，不是纯传输时延。
词典不走应用结果缓存或历史；helper有独立临时home，EOF退出后核对清理和原App树不变。
报告同时保留未达标值，不作为应用启动、取词或翻译的额外条件。
完整GUI首屏、用户设备空闲CPU/内存、长稳/休眠及真实模型A/B仍需另外验收。
首轮性能CI在构包前发现测试预期未解析macOS的`/var`目录别名，已修正canonical路径比较；
修正build163曾全部达到三项目标；最新build165的原始样本独立核验也已完成。
最新词条查询格式化最慢P95为0.151ms、常驻配置IPC1.245ms，均满足目标；
约6KB长文分类在macOS15/14分别为2.835/2.020ms，未达到内部2ms目标，macOS26为1.642ms。
保留这些差异和measured_miss，不把正确性绿灯当性能全部达标，不新增用户门槛。
不混池计算P95，启动与词典首次打开/校验另记；[逐系统P95/峰值及范围](MACOS_TODO.md#native-performance-checkpoint)保留原始失败和真实数据。

更新fixture另加入空闲CPU/内存采样：在一次性App副本中，更新检查前静置5秒，
再采31个一秒间隔的libproc快照，只统计其PID及后代。报告CPU原始计数、实际经过时间、
单核百分比的P95/最大值/全区间值，以及RSS和physical footprint的原始样本/P95/最大值。
驱动本身不计入，无模型调用；进程身份变化或数据缺失明确失败，高资源用量如实保留。
此为至少30秒的CI空闲基线，不代表用户设备、长稳、睡眠恢复或跨全部macOS系统的资源表现。
build164实际执行了31快照，但首版误将Mach ticks当ns，CPU数字已撤回。
修正保留原始ticks及mach_timebase_info分子/分母，并按真实比例换算；非1:1单位回归已加入，
build165已用实际125/3时间基取得并独立核验31.272秒/31快照：
单核CPU P95 0.074400%、峰值0.128849%、全区间0.011585%，RSS峰值46.64MiB。
采样期间稳定1个App进程；它不是完整交互负载、长期运行或用户设备功耗承诺。
