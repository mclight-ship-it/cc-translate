...
**当前可下载检查点：原生界面、图片/OCR翻译、词典来源、复制隔离、字号/摘要/历史条数/输入上限及结果位置；移植尚未完成。**
最新批次9b55942已通过[完整App与同包三系统验收](MACOS_TODO.md#native-result-position-checkpoint)。
下文保留更早失败与修复过程；双击复制间隔仍在后续源码中验证，不包含在本包内。
...
当前推荐下载9b55942保留bf89495已验图片链路，并已完成完整App同包三系统验收，见
[当前批次](MACOS_TODO.md#native-product-preferences-checkpoint)，未声称完成全部移植。
以下保留早期工程包记录：
...
9b55942已实际通过原生11项位置测试、125PNG与完整同包三系统验收，当前下载已升级至该源码。
...
实际watch exit0，完整包字节已独立核验。结果位置已经包含在当前9b55942下载中。

<a id="native-copy-interval"></a>

### 双击复制间隔（c758原生通过，尚未包含在推荐下载中）
...
待新包验证通过后，检查：输入0.75并应用、关闭/重开设置后值保留；未开启快捷键时不会偷偷监听；
...
原输入上限26和间隔25方法均通过，127PNG完整。完整App与同包三系统仍在验证，
当前推荐下载继续为9b55942，不要求用户提前测试尚未交付的功能。
...
以及真实Carbon lease和本进程合成事件路由的测试。尚待Mac实际编译/执行；
...
零失败），英中历史流程完整通过；后续完整App及同包14与26也已通过；当前下载已进一步升级为9b55942。
...
以及字号、长文摘要、历史保留条数、输入上限和结果窗口位置设置；保留明确刷新模型目录、设置/主窗口/截图共享选择、
主动纯文本粘贴、
...
producer执行755个原生测试（732通过、23构包前可选skip、零失败）并保留125张PNG，
...
- 源码：`9b5594247b7fba8e5be90622d758e858222486e6`；
  [run35324547280](https://github.com/mclight-ship-it/cc-translate/actions/runs/35324547280)；
  [完整App下载](https://github.com/mclight-ship-it/cc-translate/actions/runs/35324547280/artifacts/10539345743)；
  [原生离屏截图](https://github.com/mclight-ship-it/cc-translate/actions/runs/35324547280/artifacts/10539027822)。
- 内层`CCTranslateMac-P0.zip`：19,532,818 bytes；
  SHA-256 `567496d0fb34a9b0f72183f1f20a01bb3417a7dfcfd96c0322ab4861b9392b25`。
  artifact保留到2026-09-25T08:52:05Z；过期时只取新的经核验固定run，不使用未知镜像。
...
- Apple Silicon、macOS14+候选；Intel未支持承诺。用户不需要Xcode/Python/Git/付费开发者账号。
  ...
  结果位置偏好已包含；双击间隔、截图全局热键开关、恢复默认、Claude等仍在继续。
