...
**后续反馈正在实施：**Dock 改为仅有窗口时显示；截图选区后直接翻译，文字/图片模式放到设置；
修正浏览器/PDF 双击复制兼容、权限设置跳转，恢复作者赞助并简化菜单。
新源码尚待 Mac 验证，推荐包暂不变，见[本轮清单](MACOS_TODO.md#native-quick-translation-feedback)。
...
在Safari/Chrome等浏览器选中普通网页文字（可含链接/格式）再双击Cmd+C，
