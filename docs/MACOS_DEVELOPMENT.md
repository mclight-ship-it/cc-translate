...
**当前可下载检查点：原生界面、词典、截图/OCR、历史、关于、模型目录/手工ID与纯文本粘贴；移植尚未完成。**
...
### 明确发送图片翻译（已实现，同包验收进行中）

这是单独的“发送图片翻译”操作，不是把本地OCR文字改名为图片翻译。
当前推荐下载仍是已验证的模型目录包；图片源码及验证进展见
[图片翻译检查点](MACOS_TODO.md#native-image-translation-checkpoint)，未声称完成全部移植。
...
当前正在处理线程用法，保留取消/排空及主界面响应要求；推荐下载仍为下方已验模型目录包。
...
不会自动重放写入。实现和15项新原生测试已交付，仍需独立新源码验证，不在当前推荐包内。
...
零失败），英中历史流程完整通过；完整App及同包14与26尚待验收，当前run仍在执行，暂不替代当前下载。
...
当前新增明确刷新模型目录、设置/主窗口/截图共享选择；保留主动纯文本粘贴、
...
producer执行522个原生测试（500通过、22构包前可选skip、零失败）并保留84张PNG，
同一个App在15.7.9/14.8.9/26.6.2各通过222进程/604核心/20后置Foundation及独立About1；
...
- 源码：`d637beabc61fff9fa1af093217feed9567361293`；
  [run35229459129](https://github.com/mclight-ship-it/cc-translate/actions/runs/35229459129)；
  [完整App下载](https://github.com/mclight-ship-it/cc-translate/actions/runs/35229459129/artifacts/10502020234)；
  [原生离屏截图](https://github.com/mclight-ship-it/cc-translate/actions/runs/35229459129/artifacts/10500633203)。
- 内层`CCTranslateMac-P0.zip`：19,276,300 bytes；
  SHA-256 `1e5e3acff01ce084e96a2ff047b27598df5416ab7249a93d663439413fe3d73b`。
  artifact保留到2026-09-24T14:06:32Z；过期时只取新的经核验固定run，不使用未知镜像。
