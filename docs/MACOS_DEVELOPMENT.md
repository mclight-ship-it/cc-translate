...
**Pearl 原生界面正在接入：**按已确认的暖白/紫色设计统一翻译、快速结果、历史、词典、
设置、截图状态与关于界面，保留原生编辑器和现有工作流程；不用宣传文案或 Demo 模拟数据。
新增词典页面仅离线查询，不因未命中而调用 AI。工程和截图验收进展见
[Pearl 实施记录](MACOS_TODO.md#native-pearl-ui)。验证完成前，下列 build183 仍是已验证可用包。

**最新反馈已验证：**Dock 改为仅有窗口时显示；截图选区后直接翻译，文字/图片模式放到设置；
修正浏览器/PDF 双击复制兼容、权限设置跳转，恢复作者赞助并简化菜单。
推荐包为 **0.1.0/build183**，源码 `2fa45ee`；961 原生测试（23 构包前可选 skip，零失败）、
...
已加入系统管理的“登录时启动”及批准/关闭/刷新入口，本包另有独立Mac版本0.1.0/构建183；
...
producer执行961个原生测试（938通过、23构包前可选skip、零失败）并保留173张PNG，
...
- 源码：`2fa45ee6976571cb18e9e76301035ecac6c73d37`；版本0.1.0、构建183；
  [run35520763932](https://github.com/mclight-ship-it/cc-translate/actions/runs/35520763932)；
  [完整App下载](https://github.com/mclight-ship-it/cc-translate/actions/runs/35520763932/artifacts/10608641384)；
  [原生离屏截图](https://github.com/mclight-ship-it/cc-translate/actions/runs/35520763932/artifacts/10608801164)。
- 内层`CCTranslateMac-P0.zip`：22,369,106 bytes；
  SHA-256 `b0650922fbc8da082176f3b609fce10fde828ef3913158b1824c5fc4ec075171`。
  artifact保留到2026-09-27T16:16:47Z；过期时只取新的经核验固定run，不使用未知镜像。
...
**本轮优先复测：**退出旧版后解压替换 App；关于窗口应显示0.1.0/build183，配置/CLI/账号保持原样。
