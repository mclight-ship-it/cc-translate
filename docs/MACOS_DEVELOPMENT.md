...
**当前可下载检查点：原生界面、图片/OCR翻译、词典来源、复制隔离、字号/摘要/历史条数/输入上限、结果位置及双击复制间隔；移植尚未完成。**
最新批次c7581a8已通过[完整App与同包三系统验收](MACOS_TODO.md#native-copy-interval-checkpoint)。
下文保留更早失败与修复过程；截图全局快捷键仍在后续源码中验证，不包含在本包内。
...
当前推荐下载c7581a8保留bf89495已验图片链路，并已完成完整App同包三系统验收，见
...
9b55942已实际通过原生11项位置测试、125PNG与完整同包三系统验收；当前c7581a8下载继续包含该功能。
...
实际watch exit0，完整包字节已独立核验。结果位置继续包含在当前c7581a8下载中。
...
实际watch exit0、3jobs/42steps全通过，推荐下载已升级为c7581a8。
...
新增18项和先前两个失败方法均通过；完整App及同包多系统验证继续，当前下载仍为c7581a8。
...
正常停止现与transport failure一样退休待处理恢复请求；测试复用唯一原生按钮定位，仍待复验。
...
零失败），英中历史流程完整通过；后续完整App及同包14与26也已通过；当前下载已进一步升级为c7581a8。
...
以及字号、长文摘要、历史保留条数、输入上限、结果窗口位置和双击复制间隔设置；保留明确刷新模型目录、设置/主窗口/截图共享选择、
主动纯文本粘贴、
...
producer执行780个原生测试（757通过、23构包前可选skip、零失败）并保留127张PNG，
...
- 源码：`c7581a87f2b386df887492e31113e5e7bd92f7ee`；
  [run35330446905](https://github.com/mclight-ship-it/cc-translate/actions/runs/35330446905)；
  [完整App下载](https://github.com/mclight-ship-it/cc-translate/actions/runs/35330446905/artifacts/10541762221)；
  [原生离屏截图](https://github.com/mclight-ship-it/cc-translate/actions/runs/35330446905/artifacts/10541690381)。
- 内层`CCTranslateMac-P0.zip`：19,545,325 bytes；
  SHA-256 `73c2d718ec2ee43606480ad7e8b055955731ff59089e1b7098a5094c8ac36b67`。
  artifact保留到2026-09-25T10:03:18Z；过期时只取新的经核验固定run，不使用未知镜像。
...
- Apple Silicon、macOS14+候选；Intel未支持承诺。用户不需要Xcode/Python/Git/付费开发者账号。
  ...
  结果位置与双击间隔已包含；截图全局热键开关、恢复默认、Claude等仍在继续。
