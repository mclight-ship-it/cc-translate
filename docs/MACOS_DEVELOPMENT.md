...
**当前可下载检查点：原生界面、图片/OCR翻译、词典来源、复制隔离、字号/摘要/历史条数/输入上限；移植尚未完成。**
最新批次bf89495已通过[完整App与同包三系统验收](MACOS_TODO.md#native-product-preferences-checkpoint)。
下文保留更早失败与修复过程；结果位置偏好仍在后续源码中验证，不包含在本包内。
...
其中位置11项和关联复制原方法均通过，125PNG完整；完整App/三系统验证尚在运行。
本节位置功能仍不包含在已核验的bf89495下载包中。
...
源码新增25项原生测试和2张渲染，尚不能据此宣称已通过Mac编译或打包。
...
以及字号、长文摘要、历史保留条数和输入上限设置；保留明确刷新模型目录、设置/主窗口/截图共享选择、
主动纯文本粘贴、
...
producer执行744个原生测试（721通过、23构包前可选skip、零失败）并保留122张PNG，
...
- 源码：`bf89495cc991bc75b9cc39b263d824120a36246e`；
  ...
  [完整App下载](https://github.com/mclight-ship-it/cc-translate/actions/runs/35320398227/artifacts/10537527468)；
  [原生离屏截图](https://github.com/mclight-ship-it/cc-translate/actions/runs/35320398227/artifacts/10537112096)。
- 内层`CCTranslateMac-P0.zip`：19,522,149 bytes；
  SHA-256 `6e56b0c974273d7b9a366f63d56c9a30c8a100b806a5fffa696ab70aff0fa0c5`。
  artifact保留到2026-09-25T08:01:09Z；过期时只取新的经核验固定run，不使用未知镜像。
...
- Apple Silicon、macOS14+候选；Intel未支持承诺。用户不需要Xcode/Python/Git/付费开发者账号。
  ...
  结果位置偏好不在此包内；双击间隔、截图全局热键开关、恢复默认、Claude等仍在继续。
