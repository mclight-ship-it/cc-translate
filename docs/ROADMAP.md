...
**当前推荐包：e6ab5dc，Mac 0.1.0/build165 / [run35466603008](https://github.com/mclight-ship-it/cc-translate/actions/runs/35466603008)。**
...
真实watch0、3jobs/44steps全通过；原生890项（23构包前可选skip、零失败）、151PNG已核验，
...
三系统实际About在该检查点核对0.1.0/build146；当前推荐build165已在本轮重新核对。
...
### 已解决的 P4 验证问题（保留当时失败，不再是当前阻断）

以下155–160的失败均由build161完整闭环，不需要用户处理。
build155/156的安装fixture均在副本签名阶段失败，未产生完整新包；156的卸载排空交互
原生回归已通过。正在把Python标准库整树移出系统预留的Helpers嵌套代码目录，
同步启动器/桥接/归档和同包验收，保留深度签名验证；不能以原生测试绿色冒充升级通过。
build157已证明新布局可签名并实际安装/重启，但后续配置复核发现fixture类型断言错误，
整run仍失败；正在修正保存后规范化读回与新PID记账，再执行全部六场景及同包矩阵。
build158的正向升级及配置SHA/偏好/自有数据保留已通过，但负例断言误用了签名错误码，
整run仍失败。按固定Sparkle实际EdDSA错误链修正断言，且汇总清理已确认场景的失败；
未知清理仍硬停止，不通过绕过签名或删除负例来得到完整包。
build159正向与坏签名两场景均完整通过，其余场景被临时私钥/目录同名阻断；
正在隔离密钥目录并补清理异常的证据保留。既有签名安装步骤提前以缩短失败反馈，
完整原生/同包三系统及发布前断言全部保留，尚未更换推荐下载。
build160现已通过全部六个临时签名升级场景及890项原生回归，配置SHA/偏好/自有数据均保留，
拒绝与取消保持原App字节；macOS15和14成功。macOS26的一个旧空RTF测试却在C发布后、
AppKit计数尚未推进时开始读取，两个断言失败，完整run仍未通过。
正在修复测试的发布就绪等待，保留生产换主拒绝及全部原断言；不以反复重跑或跳过测试出包。
build161已修正发布就绪并通过完整验证，当前推荐已统一更新；P5固定样本性能、真人平台矩阵及P6授权继续。

### 已核验的 P5 自动化基线，真实设备/长稳仍另列
P5的无模型固定样本测量已继续实现：包内分类、真实词库查询/格式化及常驻只读IPC，
分开首次与100次预热后样本，并保留P95/最大值和未达标结果。build163已经完成完整三系统及
原始统计独立复核，当轮所有case的分类/词典/IPC P95目标均满足。
最新build165也已完成同包三系统及独立核验：词典/IPC最慢P95为0.151/1.245ms，满足目标；
macOS15/14长文分类为2.835/2.020ms，略超内部2ms目标，已保留measured_miss，不通过重跑挑选好数字。
启动、词典打开与SHA检查另外记录；不把warm结果当完整GUI冷首屏，
不以此冒充完整GUI首屏、长稳/休眠或真人TCC/模型A/B，也不新增使用门槛。
首轮性能CI只在便携测试发现macOS临时目录别名的预期路径问题，修正后另跑，
不将该失败轮次报告为已得到Mac性能数据。
已继续实现真实隔离App及后代的30秒空闲资源采样（5秒静置、31快照），
保留原始CPU/RSS/physical-footprint和实际时间、P95/最大值；build164六场景和采样已实际通过，
但API复核发现CPU的Mach ticks被误当ns，已撤回其CPU结论并补实测时间基换算/非1:1回归；
修正build165已实际读取125/3时间基并独立核验31.272秒样本，CPU P95 0.074400%、RSS峰值46.64MiB，
没有引用旧的错误CPU数字；
不把短时CI基线当长稳/休眠验收，也不新增资源使用门槛。
目前授权范围内可自动完成的产品开发及验证已收尾；剩余真实设备集中验收、长稳/休眠、
正式更新渠道/应用许可/发布授权单列，不自动调用用户账号或发布Release。
### 较早的实施记录（对应各自源码，不是当前待办）

当时继续Sparkle接入、更新签名资产和N→N+1保护；已核对稳定2.10.0的官方依赖及延迟启动API，
...
同包15/14/26各253process/744core/21后置Foundation通过，厂商94项原字节/权限/链接保持。
...
完整App独立字节审计及同包三系统也已通过，该轮推荐升级为build150。
该检查点没有发布feed或签名升级验证，不把当时框架/界面准备当作自动更新完成。
