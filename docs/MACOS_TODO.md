...
基线：`148f7a1`；仅独立开发分支。更新日期：2026-09-24。
...
<a id="native-translation-latency"></a>
...
- [x] 当前服务单进程、十分钟闲置回收；将无模型预热接入明确翻译意图。
- [x] Claude 只保留一个未使用的预热进程，匹配模型与提示词，使用后补充；不自动重放。
- [x] 首段流式文字立即更新，后续保留合并刷新；明显非词条输入省去词典往返。
- [x] 最近二十次请求的内存计时，只保留阶段耗时、固定分类和命中标记，不包含原文/译文/账号。
- [x] 自动化验证取词计时、首次/连续/闲置后请求、取消/退出/休眠、缓存和两端既有行为；外部应用与实体按键仍属实机边界。
- [x] 免费 Mac CI、同包验证、官方 CLI 无模型冷/热准备计时及新包交付。

源码 `8007747d9e01e451c3e53978d026bab287ba820f`；
[run35961227537](https://github.com/mclight-ship-it/cc-translate/actions/runs/35961227537)；
[完整 App](https://github.com/mclight-ship-it/cc-translate/actions/runs/35961227537/artifacts/10793790676)。
内层 ZIP **22,611,028 bytes**，SHA-256
`21976b007e33ca1eae995867daf4acab3547ccc504afe5442851c3efe5b9f9dc`。

| 检查 | 实际结果 |
|---|---|
| 正常推送保护 | 隐私扫描、2088 项完整便携/Windows 回归通过；未绕过 hooks |
| 原生界面与协议 | 1060 项，23 个有独立后置包验证的可选 skip，零失败；新计时/预热协议 19 项逐名核验 |
| 连续操作 | 七个独立进程流程及三组共 180 步随机操作通过，保留原始控件记录 |
| 同一个 App | macOS 15.7.9、14.8.9、26.6.2：各 260 真实合成进程、851 核心及 21 Foundation 集成通过 |
| 原有平台能力 | Vision 4、主动粘贴 46、关联复制 12、实际 App 只读 worker 12、About 1 均保留验证 |
| 升级与包审计 | 六种临时签名升级场景通过；独立核对包哈希、源码、资源、原生方法及同包消费者 |

官方固定 Codex、临时隔离 home、无账号/模型请求的实测：

| Codex | 冷准备 | 驻留复用 | 冷版本检查 | 缓存版本检查 |
|---|---:|---:|---:|---:|
| 0.146.0 | 743.98 ms | 0.47 ms | 318 ms | 0 ms（整数精度） |
| 0.154.0 | 769.64 ms | 0.29 ms | 272 ms | 0 ms（整数精度） |

只观察到 `initialize`、`initialized`、`hooks/list`，两次准备使用同一个进程。
脚本包装器的版本复用仅限同一已验证驻留进程；退出、文件/模型变化均重新验证。
Claude 使用一次性空闲进程，最近成功文本任务仅保留无用户文本的单一提示词配置，
普通翻译、词典和摘要可补充匹配配置；图片及结果动作不覆盖该配置。

验证过程中修正了 Swift 作用域、旧版查询次数/词典请求序列、完成事件计时元数据的测试期望，
没有取消安全清理或放宽业务结果比较。macOS 26 首次运行的旧 RPC reset 用例在进程清理时
报告 `probe_cleanup_failed`；保留失败证据，仅重跑失败消费者后全通过，App 字节未变。
这不等于已修复该偶发清理错误。Windows 原有配置/历史原子写入偶发错误同样保留记录，
针对性检查和正常完整重跑通过，未修改 Windows 行为。

验收区分：触发至取词、原生请求发出、引擎准备、模型首段、界面数据首次更新、结束。
界面数据更新不等于屏幕绘制；官方 CLI 预热不发送模型请求，不能用于宣称真实模型 TTFT。
真实模型 A/B 需要相同实际模型、方向、摘要设置、文本、缓存状态和可比网络条件。

### build202 / build206 同机请求测速（2026-09-24）

用户要求改完后实际测一轮。当前没有可用的已登录模型账号的 Mac，用户也暂时无法运行命令，
因此本轮是 **Codex 合成服务、真实客户端进程链路**的对照，不冒充真实模型翻译测速。
[run35969063991](https://github.com/mclight-ship-it/cc-translate/actions/runs/35969063991)
使用标准免费 macOS 15.7.9 arm64 同一台机器、原始 build202 和 build206 的两个已核验 ZIP，
没有重建或修改 App；测试工具源码 `9756db75bb86f892baba76d772510e82ce2f147c`。
[逐次 JSON 与日志](https://github.com/mclight-ship-it/cc-translate/actions/runs/35969063991/artifacts/10795226571)。

- 五轮交替旧/新版顺序，固定英文短句 82 字节、长文 3759 字节，固定方向与同一提示词。
- 每种输入、每个版本：五次新 helper 冷请求，十次同 helper 连续请求；共 60 次、30 对样本。
- 流式开启，结果缓存、历史记录、摘要、本地词典关闭；每次恰好一个 `thread/start` 和 `turn/start`。
- 合成 CLI 返回相同固定文本，不等待模型推理；长文仅测试长输入客户端开销，不测试长译文生成。
- 首段终点为宿主从 helper 管道解析到首个非空 delta，完成终点为解析到 completed；
  不包含剪贴板取词、Swift/AppKit 首帧或真实网络模型。
- 冷请求不提前预热；连续请求的新版本预热耗时分别记录，下面将其计入总数，未隐藏准备成本。

| 输入 / 场景 | 样本数/版本 | 首段中位数：旧 → 新 | 完成中位数：旧 → 新 |
|---|---:|---:|---:|
| 短句 / 冷请求 | 5 | 2752.22 → 2771.22 ms | 2752.46 → 2771.63 ms |
| 长文 / 冷请求 | 5 | 2715.21 → 2753.13 ms | 2715.44 → 2753.68 ms |
| 短句 / 连续，含准备 | 10 | 491.65 → 2.63 ms | 492.10 → 2.99 ms |
| 长文 / 连续，含准备 | 10 | 521.51 → 2.71 ms | 521.88 → 3.07 ms |

**结论：连续请求的客户端固定开销明显下降，约少半秒；完全冷请求没有显示提升。**
含准备的连续完成耗时配对中位数减少：短句 485.62 ms、长文 518.98 ms。
不含准备时，新版首段中位数 1.74/1.95 ms；准备本身 0.87/0.73 ms。
含准备的连续完成 P95：短句 540.53 → 36.29 ms、长文 592.09 → 4.67 ms。
样本量较小，最近秩 P95 在每组 5/10 个样本时就是最大值，不作总体尾延迟承诺。
旧新版冷请求约 2.7 秒，差异未作显著性判断；不能说所有场景都加速。

二十个 helper 正常退出，进程组清理检查全部通过；临时 home 已删除，App 字节未变。
独立重算全部聚合、样本覆盖、配对提示词/输出；新版二十次连续请求均命中版本与驻留进程缓存。
原始 JSON SHA-256：`35bfbbf48757cb54d3d3bed68f3b4f5546143d71dadbbbd43b5c67a83b8e6e8f`。
...
<a id="native-translation-preparation"></a>

### 提前准备入口实施（2026-09-24）

已实现首次复制、截图开始、休眠唤醒和监听恢复四个提前准备入口，
源码 `252e07f4ecf727dd5e41ba3541b38f52821391c6`，推荐安装包 **0.1.0/build219**。
[run36013481443](https://github.com/mclight-ship-it/cc-translate/actions/runs/36013481443)
完整结束为 success，实际 watch exit0；
[完整 App](https://github.com/mclight-ship-it/cc-translate/actions/runs/36013481443/artifacts/10815676224)。
内层 ZIP **22,613,802 bytes**，SHA-256
`d1c313874cd31089bd21ec1787b93688c787e7721b5afa49b6079004ad59f3b5`。

- **首次复制：**有效、非重复、非安全输入的第一次 Cmd+C 在取得来源身份后发出
  无参数的准备信号。此时只保留既有剪贴板 revision 基线，不读正文、不读 AX 选中文字，
  不开始翻译，也不启动取词延迟统计；真正的双击回调仍单独记录触发时间。
- **截图开始：**确认没有前台翻译冲突后，在屏幕权限/捕获/选区/OCR之前准备已配置服务，
  文本和图片模式均覆盖。取消选区或同步输入回调取消后，即使 helper 晚到 ready，
  也不得提交文字、图片或词典查询。未配置服务仍不启动 helper。
- **监听恢复：**仅从非活动状态进入活动翻译监听时准备一次，覆盖新启用、启动恢复、
  权限恢复与 AX-only 切回正式监听；两秒权限恢复定时器本身不刷新已活动监听的预热，
  诊断监听不授权准备。
- **休眠与唤醒：**休眠时关闭准备并回收空闲 helper，保留已有翻译意图；
  有前台请求时不停止或取消该请求。唤醒恢复仅适用于原有意图/已启用快捷键，
  不因为保存了 CLI 就无条件启动。旧 helper 尚未退出时排队到 stopped 后再恢复，
  不重叠创建两个 helper；关闭、取消、禁用快捷键、退出和连接失败清除待恢复意图。
- 所有入口复用同一预热槽和 540 秒配置范围节流；原有十分钟模型进程闲置回收不变。
  前台请求、未保存模型/方向、设置、历史、CLI检查、词典及模型目录操作优先；
  配置助手不因预热升级为翻译助手，失败不由后台自动重连或重放。
- 不更改现有 Codex CLI 登录/计费路径、模型、prompt、摘要顺序或 Apple 翻译路由。
  没有新增空模型生成请求，也没有声称通过提前调度降低了模型自身推理耗时。

原生测试新增首击隐私、无效/安全输入、旧监听回调、预热去重、配置/权限恢复、
两种唤醒时序、禁用与重新启用竞争、前台优先和截图取消检查。
首轮 build217 的 1072 项原生测试只有一条旧截图断言失败：它把“取消不提交”
写成了“已配置截图不启动任何 helper”。已改为同时验证已配置/未配置两种情况，
并在取消后实际递送 helper ready/预热完成事件，确认无延迟提交；不是删除取消校验。

最终验证：

| 范围 | 实际结果 |
|---|---|
| 本地针对性回归 | 既有性能/预热/打包测试 123 项通过；正常推送 hook 的完整 Python 回归 2139 项通过，没有绕过 |
| 原生 Swift | 1075 项，1052 通过、23 构包前可选 skip、0 失败；新增 11 项 `TranslationPreparationTests` 全部通过 |
| 同一 App：15.7.9 / 14.8.9 / 26.6.2 | 每系统 260 process、851 core、21 后置 Foundation 全通过，无后置 skip；运行前后包字节保持不变 |
| 既有产品回归 | 原生连续流程/三组共 180 步固定种子交互、包内 About/词典、六种临时签名升级 fixture 通过；不等于实体按键、真实账号或 TCC 验收 |
| 独立交付核验 | ZIP SHA/大小/CRC、0.1.0/build219、最低 macOS14、主程序可执行权限、Python symlink 均核对；原包直接复制，未在 Windows 解包重压 |
| 官方 Codex 0.146.0 / 0.154.0 | 仅版本与初始化预热，冷准备分别 1086.88 / 778.54 ms，驻留复用 0.606 / 0.432 ms；缓存版本检查不再起进程，没有账号/模型调用 |

[原生日志](https://github.com/mclight-ship-it/cc-translate/actions/runs/36013481443/artifacts/10814064194)、
[macOS14 同包报告](https://github.com/mclight-ship-it/cc-translate/actions/runs/36013481443/artifacts/10815763207)、
[macOS26 同包报告](https://github.com/mclight-ship-it/cc-translate/actions/runs/36013481443/artifacts/10816426025)
均与上述源码、run 和内层 ZIP SHA 对齐。

**测速边界：**本轮验证了第一次复制/截图开始即可准备、已准备会复用，以及取消/唤醒竞争下的正确性。
准备探针只是需要被提前覆盖的客户端开销，不是 GPT 翻译完成耗时，也不是新旧版本真实模型同机 A/B；
不宣称本轮快了某个百分比。此前 build202/build206 的合成测速保留为历史，不借用为 build219 收益。
仍是 Apple Silicon、macOS14+开发包，未做 Developer ID 签名/公证，不是正式 Release。
...
- 保留原始首段计时，另记录首个非标题内容与摘要后译文标题边界；
  标题边界只是输出结构证据，不冒充语义质量评估。记录仅在内存保留，不含原文和译文。
- 区分用户选择的模型配置与 CLI 返回的解析模型/推理信息，缺失则明确未知，
  不把 `auto-fast` 当成实际模型，也不把缓存命中说成本次执行了模型。
- 有界、短寿命的精确内存结果缓存与磁盘历史分离；显式重译绕过并作废旧项。
  完整输出契约与运行时身份不明或变化时不复用，取消/失败不缓存，清历史/设置/退出清理；
  OCR、图片和后续结果动作仍沿用原有不缓存策略。
- 摘要优先结果不因后续译文追加而自动滚到底部，保留用户手动滚动和选区；
  普通无摘要结果仍保留原有跟随末尾行为。
...
也没有读取或输出账号凭据。无法证明这是用户允许的 ChatGPT 订阅路径，因此本轮不在该配置上测速。
新提供的 `tools/codex_translation_eval.py` 默认只生成离线计划；真实执行需显式同意，
并拒绝 API key、自定义服务及无法确认的账号/目录。仅使用工具内原创公开中英样本，
将普通文本的当前/精简提示词配对，摘要与代码任务保持原提示词；不自动把非空结果判为质量通过。
没有真实质量与耗时证据之前，生产模型、prompt 和摘要顺序全部保持不变。

**已完成的本地验证：**源码 `eea7254`，纯合成隐私样本修正 `69626f1`；
正常 pre-push 的隐私扫描、Python 编译及完整 **2241 项**测试通过。
离线评估工具 **64 项**测试通过；持久计划包含11例/17个计划请求，实际模型调用 **0**。
关闭历史的确定性10次相同请求：强制执行为10次provider调用，内存复用为1次调用/9次命中；
这是重复工作减少证据，不是模型推理或UI耗时百分比。
[build220](https://github.com/mclight-ship-it/cc-translate/actions/runs/36131839229)
原生1105项（23可选skip）零失败，portable1248项及固定Python评估工具64项通过。
后置260项进程测试产生35个断言失败：共同的旧完成载荷断言尚未纳入model_info和memory_cache_hit；
归档和跨系统矩阵按门槛正确阻止，未发布包。现已将断言同步为精确新合同，
仍要求fixture没有实际模型确认、OCR/图片没有内存命中标志，不放宽为任意载荷。
build221进程260项已通过；后置Swift/helper的共同完成断言还需同步model_info，已按各fixture精确修正。
该轮新增摘要测试错误地等待“有选区时立刻富文本转换”；现有安全行为本来会延后转换以保住选中内容，
不应改坏产品来迁就测试。修正后等待原生视图的完成状态，再验证原文/选区/位置；
无选区时的富文本转换由另一个七语言测试覆盖。生产实现没有因这些断言问题改动。
...
- 换模型只考虑用户现有 Codex CLI 登录方式可调用的 GPT 模型；不新增付费用量 API，
  不考虑 Claude、Gemini 或专用云翻译服务作为此次替代方案。下节的跨服务候选保留为历史调查，
  不再是本轮实施建议。也不默认启用另外消耗额度的 Fast mode。
- **摘要必须先于译文**，其用途就是让用户不必阅读全文。不采用译文优先或按需摘要方案，
  不关闭或绕过已启用的摘要；普通翻译 prompt 精简实验须与摘要任务分开。
- Apple Translation 先做真实输出的质量评估，不以官方介绍、模拟译文或人设评分替代实测。
  它本身不是摘要/代码解释引擎；即使普通句子质量合格，也不能无条件替换摘要和解释路径。
  若将来研究并行生成摘要与本地译文，仍必须先展示摘要，且不能凭理论并行就宣称更快。
- 测速口径区分：Apple 单条 `translate` 返回完整结果，不是模型逐 token 流式输出。
  测试须记录完整片段返回、首次准备/下载与后续调用，不能把完整返回时间标成与 Codex
  流式首 token 相同的指标。质量按漏译/错译、否定和数字、术语、代码/结构、流畅度分别判断，
  不把“非空输出”或某个 BLEU 分数当成适合本产品的充分条件。

再次对照 Windows 与 Mac 源码后确认：Mac build206 **已经有** Codex 无内容预热、
  版本缓存、十分钟驻留和首段立即刷新。它不是“尚未移植 Windows 预热”。
  当时发现以下覆盖差异；后续提前准备入口实施见上节，仍不是模型端到端速度收益测量：

| 场景 | 当前行为 | 值得验证的改进 |
|---|---|---|
| 第一次复制 | Windows 每次 Ctrl+C 即尝试无内容预热；Mac 在组成双击 Cmd+C 后才回调 | 仅在用户启用翻译快捷键、配置已就绪时，于第一次复制提前准备进程；不提前读/传剪贴板正文，节流普通复制产生的开销 |
| 睡眠后 | Mac 休眠会停止闲置翻译 helper；只有 sleep observer，没有对应 wake 预热 | 对休眠前已有翻译意图/已启用快捷键的配置，唤醒后恢复准备；不能为未配置用户启动服务或打断当前请求 |
| 截图选区 / OCR | 开始截图时未调用翻译准备，识别完成提交时才确保服务可用 | 将无内容准备与框选/OCR 重叠；取消截图不提交模型请求，不能改变原有窗口/焦点和权限行为 |
| 新启用快捷键 / 权限恢复 | 启动已启用监听器时有应用级准备；单独 start/recovery 主要恢复监听 | 用统一的、去重的“监听器从不可用变为可用”入口触发准备，避免必须先开翻译页 |
| 长时间不用 | Windows auto-fast 不设置闲置回收；Mac 十分钟回收，540 秒范围内节流 | 保留有限驻留与功耗约束，先测用户真正的冷命中频率；不把永久常驻多进程当作无代价提速 |

尤其是 Mac 双击回调当前只尝试 `resumePrewarm()`；若 helper 已因休眠断开，
  该入口不能独自重新建立连接。新的准备入口需要复用配置/取消/前台优先的既有状态机，
  不能简单在每个按键上另起 CLI。以上是实施前的源码核实记录，后续完成情况见上节。

#### Apple Translation 真实质量与延迟评估（2026-09-24）

**已取得真实译文，不再是语言包下载窗口上的阻塞记录。**
最终语料源码 `45a082d`，成功运行：
[35985556220](https://github.com/mclight-ship-it/cc-translate/actions/runs/35985556220)。
macOS 15.7.9 / Xcode 16.4 和 macOS 26.6.2 / Xcode 26.6 两个标准 arm64 runner
各完成 56 个输出：28 个原创公开中英双向样本，各跑两遍，共 **112 个真实输出**。
所有响应的原文回显与输入一致，方向分别为 `en → zh`、`zh → en`；
同一系统上的第二遍译文均与第一遍完全一致。

质量部分是 **AI 对实际原文/译文的逐条双语审读**，不是人类盲评，不按参考答案逐字匹配，
也不把小样本结果当作总体准确率。测试覆盖否定、数字、单位、术语、混排、列表、
Markdown、代码、习语、多义词、日期、疑问、段落和五段长文。
测试器的原始 `quality_score` 保持空值，审读结果与原始报告分开保存。

**结论：本轮 system-default 路径速度有吸引力，但质量不足以直接替代 Codex 成为默认翻译。**
普通否定句、单位、疑问、习语及多数标识符保留正常；不能据此推断专业内容也可靠。
以下问题在同系统的两遍输出中均可复现：

| 样本 | 实际问题 | 范围 |
|---|---|---|
| `terminology` | `idempotency` / “幂等性”含义丢失；中译英分别出现 `power equality`、`power equivalence`，指数退避变成 `exponential avoidance` | 两系统、双向专业术语均有明显错误 |
| `en-zh-long` | 图书馆系统的 `trial` 多次译为“审判”，借书 `loan` 译为“贷款”，终端 `kiosk` 译为“售货亭” | 两系统；有上下文仍不能可靠消歧 |
| `en-zh-long` | 结束日期 `2031-10-19` 译成“203年10月19日”，丢失年份一位 | macOS 15；macOS 26 未出现这一数字错误 |
| `markdown` | 表头、分隔行和数据行之间插入空行；英译中还把对齐用的 `:` 改成全角 `：` | 两系统，Markdown 表格语法被破坏 |
| `en-zh-code` | `hello_world` 字符未改，但包围它的反引号变为引号 | macOS 15；macOS 26 保留了两个代码跨度 |

另有需要校阅的表达：中译英的费用百分比变成 `7.5% of the cost/fee`，
长文的系统试用期变成 `probationary period`。数字相同不等于数字间的关系准确。
相反，`seal` 译成不同的密封部件用词没有被仅因与参考“密封圈”不一致而判错：
英文原文没有指定部件类型，仍保留“换密封部件，而非整个罐子”的范围。

首轮运行 [35984105909](https://github.com/mclight-ship-it/cc-translate/actions/runs/35984105909)
也完成了 112 个输出，但日期样例的中文“48小时前”存在指代歧义。
审读后将源句改成明确的“将于……举行”和“研讨会开始前48小时”，再实际重跑上述最终测试。
两系统、两个方向均正确表达了未来时及活动前的截止时间，**不把首轮这个差异列为引擎错误**。
其余 27 个样例在各自系统上的首遍译文与首轮完全一致，前述术语、年份和格式问题仍然复现。

**完整响应耗时（毫秒，不是流式首字或产品 UI 端到端）：**

| 系统 / 方向 | 该方向会话首条 | 首遍后续不同短输入中位数（12 条） | 第二遍重复短输入中位数（13 条） | 五段长文首遍 / 第二遍 |
|---|---:|---:|---:|---:|
| macOS 15 / 英→中 | 1486.63 | 150.11 | 142.59 | 1056.43 / 954.72 |
| macOS 15 / 中→英 | 212.85 | 113.68 | 93.04 | 753.26 / 648.63 |
| macOS 26 / 英→中 | 1090.11 | 131.56 | 126.12 | 903.01 / 864.83 |
| macOS 26 / 中→英 | 136.57 | 88.40 | 81.30 | 580.69 / 623.45 |

- 首次语言准备至确认 installed 分别约 **23.86 秒 / 59.70 秒**，与翻译耗时分开，
  包含正常下载交互及等待语言包可用。之后反方向准备分别约 6 / 19 毫秒，不是另一次全新安装。
- 执行顺序固定为先英译中、后中译英；“会话首条”不证明系统模型物理冷启动。
  两系统在不同主机上运行，不能把表中差异归因为系统版本提速。
- 第二遍复用相同原文，可能受系统内部缓存影响；不能把它伪装成全新输入的热推理。
- 四次取消探针最终均返回取消，但从实际发出取消到 API 收敛仍约 **2.73–3.75 秒**。
  配置延迟为 20 毫秒，实际取消发出约在 39–98 毫秒。未来接入时需立即更新 UI、
  丢弃旧请求结果，不能把等待后端停止作为用户可见取消的前提。
- 没有调用真实 Codex/GPT 作同机对照，也没有测试生产 UI；因此不声称“比 GPT 快多少”
  或“质量比 GPT 差多少”。本次未增加付费 API 用量、改默认模型、改摘要顺序或发布产品包。

**策略边界与下一步：**

1. 当前请求的是 `system_default`，不是显式 `lowLatency` / `highFidelity` 对照。
   [Apple 官方 highFidelity 说明](https://developer.apple.com/documentation/translation/translationsession/strategy/highfidelity)
   指出 macOS 26.4+ 可使用 Apple Intelligence 提高质量；没有 Apple Intelligence 时会回退到传统模型。
   本次 macOS 26 主机只读查询 `SystemLanguageModel.default.availability` 的真实结果为
   `unavailable(...deviceNotEligible)`；macOS 15 的 SDK 没有该框架。
   **因此没有在这台主机上冒充高保真对照，也没有修改系统资格或启用账号。
   该高保真路径仍需在符合条件的设备上实测，不能用本次结果否定它。**
2. 现阶段优先验证上述 Codex CLI 准备时机覆盖：第一次复制、开始截图、唤醒及监听恢复。
   不改变译文模型，质量风险小于直接换成本轮 Apple 默认路径；实际收益仍需单独测量。
3. Apple 如果继续做候选，必须单独验证已启用 Apple Intelligence 的高保真能力。
   Markdown/代码需要结构化保护后重测；仅保护反引号无法修复“审判”“贷款”和日期语义错误。
   不以关键词筛选就宣称能自动识别所有不适合本地翻译的输入。
4. 摘要开启的路径继续先出摘要。Apple 文本翻译本身不提供摘要，不能为使用本地引擎而关闭摘要，
   也不能用“先译文后摘要”制造速度改善。

测试自动化方面，前几轮零输出是测试器生命周期、AppleScript 引用以及系统下载控件
无法通过常规 AX 标签/点击操作激活的问题，不是翻译质量证据。最终只在测试器自己的
已验证前台窗口中，用系统 OCR 识别明确的 Download/Done 文字，并在正常事件权限、
进程/应用标识和窗口边界再次检查通过后投递鼠标事件。没有修改 TCC、操作账号或截取整个桌面。
语言包是否就绪仍由真实 `LanguageAvailability` 决定，不把“投递点击”算成下载成功。

### 进一步提速研究：模型、提示词与本地路径（2026-09-24）

**后续用户决策见上节；本文的跨服务、API、译文优先建议已不适用于当前任务。**
...
用户在 build191 实测发现：设置展开只响应箭头；双击复制开关重启丢失；
浏览器/PDF 已复制文字仍读取失败；无选区不能打开快速输入；
权限信息中英混杂；多窗口复制侧栏导致导航混乱；整窗紫色不符合视觉参考。
之前的自动化通过不代表这些真实使用问题已解决，build191 不再视为本轮验收结论。

- 一个主窗口切换翻译、历史、词典、设置、关于；保留页面的原生编辑器、撤销与草稿。
  截图选区、快速输入及结果浮窗仍独立，关闭最后窗口后回到菜单栏。
- 无选区时打开轻量快速输入，不翻译旧剪贴板；读取失败时可直接粘贴继续。
- 设置展开改为整行原生按钮；权限状态采用简短、中英一致的说明，诊断与日常设置分开。
- 快捷键的用户启用意图与监听器运行状态分离；关闭应用不等于关闭用户偏好。
- 重新排查首次/第二次复制、剪贴板格式和异步读取，不再把用户已复制的文字简单归因为页面阻止复制。
- 视觉采用中性底色、原生侧栏材质和少量功能色，不将 CleanMyMac 参考理解为整窗铺紫。
- 新增连续用户操作与固定种子的原生随机交互验证；记录真实控件、模拟服务和外部应用/TCC 的覆盖边界。

本轮源码 `7953587e9a334b5bd8191a64f74d01c9d957765f`，**0.1.0/build202**：
[完整 App](https://github.com/mclight-ship-it/cc-translate/actions/runs/35904916359/artifacts/10772217886)；
[成功 run35904916359](https://github.com/mclight-ship-it/cc-translate/actions/runs/35904916359)；
[253 张原生 PNG 与流程记录](https://github.com/mclight-ship-it/cc-translate/actions/runs/35904916359/artifacts/10771347712)。
内层 App ZIP 为 22,575,340 bytes，SHA-256
`938e61317a8d7c8fc83d95eb8e5fb7c12ce9489199250592a71552ebdcce3e66`。

| 验证 | 实际结果 |
|---|---|
| 正常 Windows pre-push | 1981 tests，全部通过，未绕过 hooks |
| Mac 原生完整 suite | 1037 项，1014 通过、23 个需设置 App 路径的可选 skip，零失败；这 23 项在后续包内阶段另行通过 |
| 连续用户操作 | 七个独立原生进程流程通过；普通流程 18 步，三组固定种子各 60 步，共 180 步，每组覆盖 20 类操作各三次 |
| 本轮精确方法核验 | 88 项关联复制/偏好/reader 检查；12 项实际 App worker 检查；25 项展开/查找/权限/连续流程/窗口归属方法逐名开始并通过 |
| 同一个 App 三系统 | macOS 15.7.9 / 14.8.9 / 26.6.2，各 253 process、744 core、21 Foundation；附加 About 1、Vision 4、纯文本粘贴 46、关联复制 12、worker 12 通过 |
| 升级隔离验证 | 安装重启、坏签名、错误密钥、下载取消、安装取消、下载错误六场景符合预期；使用临时签名和本地 feed，不是公开发行 |
| 独立包核验 | 固定 source/run、ZIP/tree hash、790 项清单、86 份资源摘要、随包源码与 arm64 Mach-O 一致，两个消费者运行后包未变 |
| 视觉检查 | 253 张实际 SwiftUI/AppKit 渲染；交付图库精选 23 张及浅/深总览；检查并修正窄浮窗按钮叠字和异常启动英文诊断泄漏 |

复制现在优先读取本次手势的新剪贴板内容，不再要求浏览器必须先提供 AX 选区对象；
支持 PDF/文件元数据与有效文本并存、富文本、分项文本及延迟提供的数据。
仍校验来源进程和新复制版本，不翻译手势前的旧剪贴板，不注入额外复制键。
快捷键偏好保存用户启用意图，退出应用或权限临时不可用不清除此偏好。
七个流程使用真实原生窗口/控件与模拟服务，不是实际账号/模型、Safari/Chrome 物理按键、
首次系统授权或 VoiceOver 的完整实机验收；这些边界仍需在用户设备上确认。
以下保留未交付中间构建的排错记录，不以它们替代上述最终结果。

首轮 `3166ca3` / build192 / run35869954005 已完成原生编译并执行 984 项测试：
23 个原有可选 skip，11 个失败断言，**没有作为修复包交付**。
其中包括异步恢复最小化窗口的等待、旧的“无选区弹错误结果”断言，
以及深色截图恢复页的计数提示对比度和小字号 OCR 检查。
继续保留并更新相应行为断言，改善提示对比度，不通过删除检查掩盖问题。
该轮 producer 还达到原 30 分钟上限；后续同一标准免费 runner 保持所有验证和发布阻断，
将有界时限设为 40 分钟，并把这五个回归方法加入原九项快速检查（共十四项）。

第二轮 `1d301a0` / build193 的十四项检查仍有截图计数的整图 OCR 失败：
实际 PNG 和原生标签区域检查均可读，改为保留原图、分块识别，不向 OCR 提供期望文字。
第三轮 `52a034b` / build194 已通过该检查并实际执行全部 1029 项原生测试，
23 个原有可选 skip、39 个失败断言（含 2 个 unexpected），**仍不交付**。
新增连续流程检查暴露了原生控件查找、AX 与键盘行为问题；复制样例还暴露了
NSPasteboardItem 的单粘贴板归属以及原生文本/PDF 复制测试问题，逐项修复后继续实跑。
权限/模型状态的可读性和旧缺选区测试也按新契约修正，不能把测试已添加当成测试已通过。
快速检查失败时继续收集完整 suite 的证据，但该失败仍会阻止归档和发布。

后续 `8e09999` / build195 发现新测试引用了不存在的控件成员，`654fe83` / build196
发现 AppKit 导航顺序数组在真实分段控件/滚动条上的 Swift 桥接陷阱；均修复测试基础设施，
不放宽发布门槛。`6652046` / build197 执行 1033 项原生测试（23 skip、23 failures），
复制/监听偏好及权限本地化相关测试通过，完整用户流程仍未通过，继续修复。
流程验证区分菜单“已消费按键”和“已执行操作”，不再把返回值本身当作重复提交；
原生编辑器和结果窗口操作补充直接可访问的控件身份/帮助，编辑菜单补齐“重做”。

`12047f9` / build198 剩 9 个失败断言；`dd9ff89` / build199 已通过 1033 项原生测试
（23 个可选 skip），七个独立进程用户流程及三组各 60 步的随机交互全部完成。
修正了测试的撤销分组与原生键盘遍历，去掉紧凑设置中重复的词典标题，保留完整可见性断言。
随后代码复查发现两个真正的窗口归属问题：切页后关闭主窗口没有取消原任务，
以及快速翻译的后续请求失去结果浮窗归属。`4802a1c` / build200 新增三项回归并通过，
其余两个失败断言来自旧 About 独立窗口关闭契约；更新为单工作区关闭应取消其翻译，
但不能取消截图任务或改变外部编辑器。`39c9539` / build201 的 1036 项原生测试
（23 个可选 skip、零失败）及全部用户流程通过。

最终截图检查仍发现结果浮窗图标按钮叠画截短标题，以及异常启动时用户页面泄漏英文诊断。
`7953587` 将图标按钮的视觉标题清空但保留完整无障碍名称/帮助，增加原生属性回归；
启动异常改为中英恢复提示，内部原因仍留在诊断。权限截图使用正常模拟连接，
异常状态另行独立覆盖，不把测试环境缺包当正常设置截图。build202 完整验证已通过，见上表；
此前各中间构建均未作为本轮修复包交付。

## 当前实施：正式原生功能已接线，继续 P5 与设备验收

2026-09-16 用户已确认普通翻译链路成功，同时明确指出产品界面尚未完成，要求继续按计划实施。
此前“正常使用”的交接过早：当时可用的是后端链路，不是全部Mac产品。
现在P2/P3所列产品功能及P4开发包生命周期已经实现并有同包自动化证据；
P5固定样本和短时空闲测量已验证，设备交互、长稳和正式发行未冒充完成。
下方早期切片记录保留其当时范围。

- [x] 独立原生翻译窗口：输入/结果、方向选择、一键翻译、流式取消、双语复制/重新翻译，
  默认不再进入诊断页，不要求手动Start/Enable/Save一串前置步骤。
- [x] 菜单栏入口、可选择/复制的稳定结果浮窗、结果召回、关闭窗口不退出。
- [x] 原生设置/历史界面（本轮统一为主窗口内切页）：记住CLI选择、自动定位、中英语言/系统深浅色、历史分页追加/复用；
...
### Pearl 原生界面接入（2026-09-22，已验证）

上一轮采用浅暖白/深紫色方案实现 SwiftUI/AppKit 界面，不使用 WebView，
不把原型的模拟翻译、权限或截图控件带入产品。页面标题只描述功能，不使用宣传口号。
该配色及独立窗口侧栏已被 2026-09-23 用户反馈要求修改，见上方本轮重构。

- 共享颜色、卡片、导航与主按钮样式；保留系统/浅色/深色设置，并响应增强对比度及降低透明度。
- 翻译、历史、词典、设置和关于采用同一侧栏；窄窗口收为图标导航。
  沿用各功能已有的可复用窗口，切换入口不清空草稿，也不改变关闭最后窗口才隐藏 Dock 的规则。
- 快速结果仍是独立浮窗，新增置顶切换和打开翻译窗口按钮，保留草稿及滚动/选择状态；
  截图入口仍直接开始选区并自动翻译，不增加确认页。
- 新本地词典页面复用真实 helper 的离线查询及下载管理；未命中、未安装或出错时明确显示状态，
  不自动转为 AI 请求，也不改写翻译草稿或历史。
- 修复新布局下窗口最小尺寸被重置、浅色非活动按钮文字不可见、设置及截图恢复按钮被挤出的问题；
  关于页的构建详情使用可聚焦的原生展开按钮，不在首页堆放技术信息。
- 超长查词输入在发送前给出输入提示，不截断文本、不关闭共享 helper，也不打断翻译。

已验证源码 `1f8d9ed8361572517abb09a5098622b1cc5a476f`，**Mac 0.1.0/build191**：
[完整 App](https://github.com/mclight-ship-it/cc-translate/actions/runs/35693532432/artifacts/10680028105) /
[完整 run35693532432](https://github.com/mclight-ship-it/cc-translate/actions/runs/35693532432) /
[194 张原生渲染图](https://github.com/mclight-ship-it/cc-translate/actions/runs/35693532432/artifacts/10680380825)。

- 正常 pre-push 的 Windows 1981 项测试通过；原生 983 项（960 通过、23 构包前可选 skip，零失败）。
- 真实词典下载/安装与独立查询通过，未调用 AI、未改写翻译草稿或历史；
  同源模型到原生离屏视图的 10 次 warm lookup，P95 76.29 ms，不含下载，也不是物理桌面端到端计时。
- 同一完整 App 在 macOS 15.7.9、14.8.9、26.6.2 通过既有运行矩阵，六个临时签名升级场景通过。
  本机独立核验源码、ZIP、Mach-O、资源及三系统 receipt，确认未重建/重签替换测试包。
- 内层 App ZIP：22,520,928 bytes；
  SHA-256 `c45c37aa5af83ab87e1cab86119971caa6664255c10fead549940ddbfd0cab69`。
- 截图是实际 SwiftUI/AppKit 内容视图渲染，使用测试数据，不是 HTML 原型。
  真人键盘、VoiceOver、TCC 和正式发行仍按原计划跟进，不把本轮 UI 完成写成全部移植完成。

<a id="native-quick-translation-feedback"></a>

### 快捷翻译反馈已验证：菜单、取词、Dock、截图与赞助

本轮修正 build174 实测反馈，推荐下载已升级为 **Mac 0.1.0/build183**。

- 菜单动作统一为纯文字，保留开关勾选；移除翻译入口的 Cmd+N。
- Dock 仅在有产品窗口打开时出现；最小化或隐藏不是关闭，关闭最后一个窗口才回到菜单栏模式。
- 修复浏览器/PDF 的复制时序：在原始按键转发前记录基线，兼容第二次复制去重、
  同一版本稍后才提供文字，以及附带 PDF/图片/厂商元数据的正常文本。
  独立进程读取延迟提供的剪贴板数据；同一截止时间内等待，不额外发送 Cmd+C，
  不读取手势前的旧内容，不清空或改写用户剪贴板。
- 截图选区后直接翻译；在设置“快捷键”中选择“识别文字”或“发送图片”。
  默认本地识别后发送文字；选择发送图片时不等待 OCR，识别失败不自动改发图片。
  不再要求先确认预览；等待或失败时使用小型状态窗口，支持取消、重新截图和关闭。
- 恢复“请作者喝杯咖啡”，复用 Windows 版现有作者赞助资源。
- 从菜单启用双击 Cmd+C 而缺少权限时，打开设置“快捷键”；已有设置窗口也直接切到该页，
  不新建重复窗口，不清除其他设置草稿。

源码 `2fa45ee6976571cb18e9e76301035ecac6c73d37`：
[run35520763932](https://github.com/mclight-ship-it/cc-translate/actions/runs/35520763932) /
[完整 App](https://github.com/mclight-ship-it/cc-translate/actions/runs/35520763932/artifacts/10608641384) /
[173 张原生内容视图截图](https://github.com/mclight-ship-it/cc-translate/actions/runs/35520763932/artifacts/10608801164)。

| 验证 | 结果 |
|---|---|
| 正常 Windows pre-push | 1981 tests，隐私扫描通过，未绕过 hook |
| 原生完整套件 | 961 tests，原有 23 构包前可选 skip，零失败；完整源码方法清单逐项核对，520.605 秒 |
| 本轮行为 | 自动截图 24 项、Dock 15 项、设置反馈 5 项通过；复制/剪贴板/选区模型及新 worker 合计 61 项逐方法通过 |
| 图片与赞助 | 173/173 PNG 完整；实际原生赞助图的两个二维码与原图解码结果一致；包内原图 262177 bytes，与源码逐字节一致 |
| 完整工作流 | 3 jobs / 44 steps 全成功；同一 App 在 15.7.9 / 14.8.9 / 26.6.2 各通过 253 process / 744 core / 21 Foundation |
| 独立包审计 | 实际版本 0.1.0/build183、ICNS、来源、790 清单项及 86 资源 hash 核验通过；完整 App 仅下载一次 |
| 更新与性能 | 六个临时签名升级场景及同包性能、空闲时间基/原始样本核验通过 |

内层 ZIP 为 22,369,106 bytes，SHA-256：
`b0650922fbc8da082176f3b609fce10fde828ef3913158b1824c5fc4ec075171`。
下载与操作见[当前开发包](MACOS_DEVELOPMENT.md#native-translation-user-check)。

失败候选没有作为推荐包交付：build176 修正测试 extension 作用域后取消；
177 因测试 helper 可见性编译失败；178 定位原生控件查找、非激活窗口焦点及中文 OCR 测试问题；
179 修正分段控件可见区域的几何断言；180/181 证明无桌面 XCTest 宿主可同时 hidden 和 active，
改为检查 Hide 保持及没有新的 unhide/activation 通知，不要求无关的 active=false；
182 的原有输入上限失败态测试改用已存在的异步原生控件就绪查找。
183 的九项定向检查及后续完整 961 项全部通过；保留业务行为断言，
无效的宿主假设改用对应原生状态/事件验证，不以图片存在代替方法通过。
期间两次 Windows hook 遇到历史/配置原子替换的间歇 WinError 5；
原有定向 16/10 项及之后完整 1981 项通过，未修改 Windows 业务代码或跳过 hook。

截图使用 fixture 数据，不包含实体 Dock、系统标题栏或真实权限弹窗。
网页/PDF、TCC、IME、VoiceOver 仍需实机覆盖；默认未启用监控的空闲报告不代表
启用双击复制后的长期功耗已验收。没有发布正式 feed/Release，整个移植与设备集中验收仍另列。

<a id="native-compact-settings"></a>

### 前置体验调整：Dock 与轻量设置（2026-09-20）

用户明确保留双栏翻译窗口，但产品定位仍是小而美的快捷工具。窗口退到后台后应能从 Dock 找回；
设置不应要求普通用户先理解配置回读、字节预算、进程或剪贴板协议。

已使用现有 Windows `capture_ui_v2.py` 隔离工具重看真实设置/快速输入界面（中英内容未替换为网页原型）。
Windows 的主要可复用经验是短标签、紧凑分组和按需帮助，而不是复制其皮肤或删除 Mac 权限提示。

- 正常应用增加 Dock / 应用切换入口，保留菜单栏；重新打开时找回现有窗口或打开翻译窗口。
  不自动提交翻译，独立剪贴板 worker 不成为 Dock 应用，默认启动不增加业务 I/O。
- 设置按“翻译 / 快捷键 / 外观 / 更多”分组；默认显示翻译，草稿在分类切换后保留。
- 常用模型选择保留，自定义模型及安装/版本信息按需展开；已编辑的自定义草稿自动展开。
- 保留权限入口、当前错误和恢复操作、历史缩减/词典删除确认；不改配置保存、模型请求、
  8192 字节限制、进程隔离或剪贴板读取规则。
- 正常输入只显示字数；超过字节容量时仍明确显示限制和错误，不截断原文。
- 词库、快捷键、登录项和截图预览去掉常驻的实现说明；技术信息仍有对应入口和开发文档。
...
### 当前优先修复：结果窗高度和浏览器双击复制（2026-09-19）

用户在build150实测基本翻译可用，但空选区/失败时结果窗异常高、越界且难以缩小；
浏览器双击复制也经常取不到字。这两项先于后续P4实施；不要求重装CLI或更改账号。

- 结果窗原本已有resizable，问题不能用再加一次标志解决。新实现关闭该窗口的
  NSHostingView自动尺寸约束，由窗口控制视口；空态移到可滚动overlay，长状态提示有界滚动。
  打开、缩放结束及显示器变化时按visibleFrame收回，最大尺寸随屏幕更新，
  小屏可低于通常最小尺寸；流式更新不重跑用户选定的鼠标/居中定位。
- 复制候选从本次双击第一键的counter开始关联，只有第二键确认且焦点/进程仍匹配才读正文。
  已完成的新复制不再错过，也不再等待浏览器可能为空/缓慢的AXSelectedText；
  没有新复制时保留AX路径，AX空值/暂不可用也可以等待本次新复制。
  第二键后仍为0.5秒有效期，单键不读正文，不接受手势前的旧剪贴板。
- 普通文本旁的HTML、浏览器来源及扩展元数据不再导致整项拒绝，只读取明确的plain-text表示；
  文件、图片、concealed/transient/auto-generated标记、多项及rich-only仍拒绝，
  不读附加表示的正文，不改写用户剪贴板。权限、安全输入、来源变化、取消及跨请求检查保留。
- 新增实际NSPanel空态/失败缩放、流式长结果滚动、小屏及显示器通知回归，
  英中底部操作/错误文字原生渲染检查，以及复制早完成、AX空值、超时旧值和浏览器元数据正负例。
  同包fresh-copy方法清单从7增至9，原真实方法仍逐项验收。

首源码`6ff2a7d915a7777b92262e02bbba137b68444236`正常privacy/两文件编译/full hook
1947项、104.975秒通过；目标便携63项通过。
[run35449559311](https://github.com/mclight-ship-it/cc-translate/actions/runs/35449559311)
已实际编译28.31秒，873项原生测试/23既定构包前skip/3断言失败（0unexpected），536.111秒。
四项新增实际窗口缩放/滚动/屏幕恢复及所有复制测试通过；145PNG库存完整，
但旧短结果操作图片中的summary被新状态区多占的空间挤出视口（浅/深色各一断言），
新中文错误图的小号次要色文字虽实际可见，却未被OCR读到（另一断言）。未当作新包通过。
...
修正源码`0812d47`的[run35293312910](https://github.com/mclight-ship-it/cc-translate/actions/runs/35293312910)
也已实际退出1，但本次编译成功，680项Swift真实执行（23项既定构包前skip）。
逐冻结源码核验，producer的11项worker、原46项粘贴、7项fresh-copy全部started/passed各一次，
整个原日志未出现旧后台promise线程警告。232进程/544.021秒、648核心/4.853秒及构包后
21项Foundation/186.173秒均通过；886便携/12.697秒通过。
失败来自2项字体方法中的8个断言（1 unexpected）：字号变更丢失主输入框未提交的组合文字、
标记区和选择；另一个原生字号选择器测试没有找到预期菜单项，动作失败后等待超时。
这是待修的实际问题，不能只改测试期望或删除组合输入断言。其余14项新字体方法已通过。
本run完整Swift耗时430.407秒；正常archive gate拒绝发布App，14/26未运行，无同包验收。

本源码截图artifact10526819862包含115PNG：前112个预期名称及3张新增150%截图，
父已逐文件核验名称/尺寸/CRC/SHA，并直接查看英文主界面、中文历史和中文OCR三图。
截图看起来正常不代表组合输入/原生操作已通过；当前仍保持原推荐包。
0812d47正常push曾被旧Windows历史原子替换WinError5拦截（2fail/89.508秒）；
...
上述`6e2a341`整轮已实际watch exit1；903portable/18.462秒、232process/522.490秒、
665core/7.330秒及21后置Foundation逐方法通过，失败仍仅为构包前的私有板fixture。
...
### P3 双击复制间隔：c758原生与完整同包三系统验收通过

设置的“划词快捷键与权限”区域新增秒数输入及“应用间隔”，沿用业务配置`double_press_window`，
默认0.5秒；支持正小数和整数，不局限于速度选项白名单或0.2–1.5的建议范围。
可写范围仅受既有JSON数字预算限制。复用已有数值偏好的输入/保存/回读状态机，整数设置行为不变。
实际monitor、成对状态机和第二次复制的baseline判断使用同一个确认后的间隔；
剪贴板新鲜度及第二次按键后的等待时间仍独立固定0.5秒，不随双击间隔扩大。

只有确认后的配置读回才改变当前时序；编辑、保存ack或尚未确认的写入不会提前生效。
改变时序清除旧半对按键、等待中的复制和来源缓存，隔离读取中的迟到结果；
相同值回读不清空有效的待处理复制。既有监听的开关/注册保留，不偷偷启用监听或回退。
保存只改一个配置键，不保存未提交的方向/模型草稿，不取消、重放翻译或改动历史。
失败保留当前已生效间隔和用户输入，明确回读恢复；无效已保存值不静默修正。
本机UserDefaults仅缓存上次确认值以支持重启后零业务I/O的原生监听；
不是伪造的配置确认，下一次helper配置读回优先。诊断的持久化opt-out继续生效。

新增25项测试（Support11/模型11/真实原生交互2/渲染1），预期原生总数780，
保留23项构包前可选skip；预期增加2张中英深浅色PNG，总数127。
交互复用实际原生文本编辑和单次按钮事件路线，不通过模型setter冒充点击。
Windows编辑器无Swift测试提供器；c758现已完成真实Mac编译/运行及完整同包三系统验证，
已取代9b55942成为推荐开发包；更不代表P3–P6完成。
...
127PNG的库存、CRC、尺寸和IDAT已独立核验。完整run实际watch exit0已读取，
attempt1的3jobs/42steps全部success；同一个App在15.7.9/14.8.9/26.6.2各通过232process/665core/
21后置Foundation（每方法一次、零skip），以及About1、Vision4、主动粘贴46、关联复制读取7和实际worker11。
producer另有903portable/20.472秒、232process/521.680秒、665core/5.261秒。
三系统HTTPS证书、SQLite、取消、EOF、不可变和临时清理全部通过。
完整[artifact10541762221](https://github.com/mclight-ship-it/cc-translate/actions/runs/35330446905/artifacts/10541762221)
已实际下载一次并独立审计：内层zip19,545,325字节，
SHA-256`73c2d718ec2ee43606480ad7e8b055955731ff59089e1b7098a5094c8ac36b67`，
tree`383d32bf640966359c782efd5c60c2763aaeb4feb554943aae499a10cb95de4a`；
690库存、80资源、52源码路径（51唯一）、6实际Mach-O及27许可均核验。
macOS14小报告artifact10542278248和26小报告10541468890也已分别下载和独立审计。
当时推荐包升级至此源码，新增双击间隔；此历史包不包含后续截图快捷键，不把一次回归通过声称为永久无时序问题。
...
首轮`4fa56e06b3ad9f0d6d01729584c53b31684ec34d` /
[run35332402808](https://github.com/mclight-ship-it/cc-translate/actions/runs/35332402808)
正常privacy/full hook1846项/97.798秒通过，实际编译35.64秒；
798项/23构包前skip/3失败断言（1 unexpected）/550.045秒，130PNG完整。
18新增方法中17通过，包含实际Carbon双lease/本进程路由、全部模型与控制器、双语开关/重开及渲染。
另一交互方法的英文冲突/释放重试已实际通过，中文注册重试的107×20原生按钮可见，
但OCR未匹配其标题；不是注册或截图链路失败。独立fixture改为读取唯一可见的公共AXButton角色，
不按enabled状态选控件、不设置state，保留单次真实原生点击与前后状态/持久化断言。
另一旧纯文本粘贴截图的中文等待语句被OCR漏识别，实际PNG中完整可见，真实stoppingAction断言通过。
完整设置快照随新增section增高以容纳底部；中文OCR保留前三个实际候选而不提供/替换期望文字，
...
第三轮`f8a014b` / [run35335961133](https://github.com/mclight-ship-it/cc-translate/actions/runs/35335961133)
正常hook1846项/97.517秒通过，实际编译33.22秒；798项/23skip/1失败（0 unexpected）/
440.728秒，131PNG完整。新增18方法全部通过，包含中英注册/释放重试的真实单次原生点击；
仅余旧完整设置图片方法失败。阶段诊断明确为`title=true, message=false, stopping=true`：
实际OCR把“等待剪贴板任务结束”读成“等待剪贴板任务纯束”，不是控制器未进入停止状态；
不同“文件任务”语句的负对照未误匹配。修正采用测试绘制时每point两像素的原生bitmap，
保留原来完整Form尺寸、语句精确断言、OCR规则与负对照，不做近似匹配或替换识别文字。
两幅中文完整设置图及负对照图按此方式绘制，并断言、记录真实像素尺寸；其余绘制路径保持。
新修正待实际Mac验证；f8尚未结束整体run，不把局部测试或图片库存当完整App通过。

f8整体watch现已实际exit1，失败日志与逐方法证据已归档；producer903portable/14.987秒、
232process/498.686秒、665core/6.284秒及后置Foundation21方法各一次通过、零skip，不替代原生失败。
第四轮`18db461` / [run35337197245](https://github.com/mclight-ship-it/cc-translate/actions/runs/35337197245)
正常hook1846项/101.799秒通过，实际编译27.35秒；798项/23skip/1失败/546.318秒、131PNG完整。
原中文停止状态图片断言及截图快捷键18项全部通过，但扩大到其他中文截图的绘制路径引入另一处
...
完整run已实际watch exit0，attempt1的3jobs/42steps全部success。
producer另通过903portable/19.153秒、232process/538.041秒、665core/5.881秒；
后置Foundation21方法各执行一次、零skip。同一个App在15.7.9/14.8.9/26.6.2各通过
232process/665core/21Foundation，以及About1、生产Vision4、主动粘贴46、关联复制读取7和实际worker11。
三系统HTTPS证书、SQLite、取消、EOF、不可变与临时清理均通过；未调用账号或模型。
[完整App artifact10545236397](https://github.com/mclight-ship-it/cc-translate/actions/runs/35338542005/artifacts/10545236397)
只下载一次，逐字节独立审计通过：内层zip19,561,280 bytes，
SHA-256`e1b9a82244323f1474711cdd874185f4b7e0f7f433e9e3a8600a6187cd8c7af8`，
tree`5d82b65af08bb394a0ee26c62538ad7f18ced9ea7d01442c888153c272aa099d`；
...
在自然退出成功且输出排空后才返回成功；首个正数写入立即标记可能已提交。
...
helper源码3e53aec已正常push，privacy/10文件compile/1925完整hook/128.765秒通过；
run35352153748已真实编译并通过816原生/23既定构包前skip/零失败及134PNG，
新5个Swift连接方法逐项通过；253process/744core及同包消费者仍待此源码结果。

下一源码已继续接入实际原生设置：Codex/Claude选择、各自CLI路径、模型预设/精确自定义ID与
未应用草稿；先保存并读回服务，再排空旧helper并绑定新服务，不重放旧翻译。
冷启动读到Claude时先重绑再发送待提交意图，本地词典命中不需要CLI，未命中升级所选服务；
截图/图片、OCR及结果动作也绑定该服务。Claude不调用Codex模型目录，保留别名和手填ID。
恢复默认扩展为包含两个模型键的12项业务设置，保留两套CLI路径/账号/系统权限/历史/词典。
UI源码da41955正常privacy及1925完整hook/100.580秒通过。
首run35354152358实际watch exit1：两个新增测试构造设置视图时漏传既有的
showDiagnostics/showAbout回调，引起测试编译错误；原日志保留，没有把生产构建或旧绿灯当成UI通过。
已补齐两个测试构造参数，没有删断言；同时修正Claude协议失败提示仍写Codex的接线遗漏，
保留Codex原提示并新增Claude错误身份/不重放断言。
现有16项新Swift产品/状态/真实原生菜单/渲染用例，含英亮/中暗两张Claude设置图。
修正源码仍待自己的Mac编译及完整同包验证，不能使用3e或ceae的绿灯替代。

第二轮250e8c7/run35354773956已实际编译成功（38.06秒），
832原生测试实际执行，23个既定前置可选skip、6个断言/异常失败（4个方法），不能算通过。
新Claude路径/模型/草稿/图片及错误身份等用例已实际运行；其中发现正常stopped事件没有清除
尚未确认的服务切换，现已补齐，并扩展原用例覆盖保存与回读两个断连时点。
原生菜单测试的截取设置Form未观察模型，现改用ObservedObject宿主并补逐阶段控件诊断；
保持实际NSMenuItem动作、保存后回读及显示选择全部断言，不改为直接调用model模拟点击。
两个既有图像断言分别将“生成摘要”识别成“生成摘更”、keys识别成kevs；
对应图使用已有高分辨率原生渲染路径，保留原文断言、不接受错字。
136张PNG完整性独立校验通过但不等同测试通过；Windows取证脚本输出中文时的编码异常也已修正，
直接使用已经下载的日志/图像继续取证，没有重下证据或把收集失败混同为产品结果。

helper前置3e53aec/run35352153748另已实际watch exit0、3jobs/42steps全success。
新16portable/4Darwin helper/5Swift逐方法各一次passed，producer982/15.340s、
253process/530.305s、744core/6.132s、816native/562.273s及134PNG通过。
完整App artifact10551003576只下载一次：19,599,731 bytes，
SHA256 `dae2e4b5514715ba5921c08c7ddaaffa0a0c3a6c78e717f46b6b8789cbaaa0db`，
tree `cb7929a5b32fa23c67ce09c81769c08306d372e4b0539f374ade80759da2ff72`；
693库存/83资源/55source路径（54唯一）/6 Mach-O/19运行时许可核验。
14/26小报告10551463449/10551248571，同一App完整审计通过；复核hash后只删除内层ZIP并确认不存在。
这些仅证明前置helper/业务链，不替代仍失败的后续UI源码验收。

第三轮ff30204/run35356605802实际编译24.34秒，832项原生测试/595.840秒：
16项新Claude产品/原生菜单/渲染全部通过，服务保存与回读断连修复及原粘贴OCR失败也通过；
但旧摘要中文说明的OCR仍有1项失败（23个既定前置skip），不能当整体通过。
单纯增加截图分辨率未解决误读；现抽取复用原粘贴测试已使用的分块识别、
UI语言校正和实际候选读取，保留全部原文断言、不注入预期字串，并加入“生成译文”不得
命中“生成摘要”的独立渲染负例。预计137张图，原16新产品方法及832总数不变。
会话取证脚本原先虽列出旧方法失败却exit0，现同时检查全局零失败汇总和各方法状态；
真实ff失败日志已验证exit1，3e完整通过日志仍exit0，没有更改GitHub原生发布门禁。
ceae3fb/run35348803751已实际watch exit0、3jobs/42steps成功，完整App10548873788与
同包15/14/26独立审计完成；库存693/资源83/source55（54唯一），本地ZIP精确复核后删除。
这些后端证据不等于下一源码的Claude原生界面已经验收；
...
0812d47已实际编译并执行16项，新3PNG也已生成；14项通过、2项失败，详见上方原始结果。
正在修字号变化的组合输入保留及原生选择器交互；下一份源码必须重新运行，
不引用旧方法通过代替修复验收，也不代替真实IME、VoiceOver或完整P3验收。

修复已交付并完成父侧源码审阅：主输入/OCR改用同一个稳定AppKit编辑器，
把尚未提交的marked text留给原生控件，只有真实外部内容替换才重写正文；字号更新仅改属性，
不再触发SwiftUI TextEditor用旧绑定覆盖组合输入。原失败断言保留并增强，新增OCR连续缩放/
候选提交及清空/历史替换2方法；字体共18方法。选择器测试先展开真实菜单并记录标题/scalars/action，
再执行实际NSMenuItem动作，不用直接改模型冒充选择；旧日志不足以证明窄空格是根因。
合并本地bundle/runtime/summary/config现有120项/14.862秒通过，原生697方法静态库存已核对，
随后以`569bca6`真实执行，结果如下；不会把0812的部分通过当成本修复结果。

`569bca6`/[run35296221920](https://github.com/mclight-ship-it/cc-translate/actions/runs/35296221920)
已实际退出1：697项Swift/353.760秒，23项既定构包前skip，6个方法有10个失败断言（2 unexpected）。
原组合输入和原生菜单两项失败此次均通过；字体18方法中16通过，但共享编辑器的主输入/OCR
两项滚动测试出现负的滚动位置。正在补齐与既有结果视图一致的文档布局计算，再恢复阅读锚点；
原失败断言保留，并加强初始长文可滚动及同一段落保留检查，修复仍需新源码Mac验证。
摘要另有4方法失败，详见下一节。

逐冻结源码核验，本run的11 worker、46粘贴、37关联复制方法均started/passed各一次，
全producer日志无旧后台promise警告；886便携/18.141秒、232进程/504.417秒、
648核心/4.981秒、构包后21 Foundation/183.490秒均通过，后置无skip。
正常archive gate仍拒绝发布，未产生App artifact、14/26未执行，推荐包不变。
UI artifact10527484670实际116PNG而非预期118；现有图片尺寸/CRC/SHA已核验，
但不能以图片存在代替控件或方法通过。
...
更改只影响后续请求，不取消/重放已提交翻译，不改已有结果或历史；400字符门槛、自然语言分类、
短文/代码/OCR例外、共享缓存签名及手动“生成摘要”操作全部不变。
保存/读回失败明确显示未确认，提供主动重读，不自动重写。About过时的图片/模型未实现说明已同步纠正。
新增15项Swift（11模型、2真实checkbox交互、2渲染），预期3张中英/失败状态PNG；
共享fixture只补规范化摘要键和Labs迁移标记，不删旧断言。新测试/PNG均未在0812运行，
随后569bca6实际执行15方法：11模型通过，但2原生交互及2截图方法未通过；
共同失败是测试无法按当前定位方式取得`NSButton`，正在检查真实控件/标识映射，不用直接改模型替代操作。
英文浅色、中文深色两张确认态PNG未生成；失败态PNG虽生成，对应方法仍有失败断言，不能称为通过。
下一份修正源码需重新验证完整字体/摘要交互和预期截图，并继续同App三系统门槛。
...
旧零值/负值如实显示，可明确纠正。无CLI也可保存；失败后仅主动重读，不自动重写。
历史与输入长度共用整数偏好状态逻辑，历史降低确认、修剪时机及原21项测试保持不变。

主输入、选中文字、OCR编辑及词典路由共用预检：原始Unicode码点数与8192 UTF-8字节预算分别检查，
空白、CRLF和组合标记计入，不截断或归一化。超限在业务请求前提示，不误报词典损坏，也不覆盖已有结果。
UI分别显示两种计数；提高字符上限不扩大字节预算。准备期间及词典未命中后的新模型请求重新检查，
已提交请求不取消/重放；真正配置快照由helper捕获，不把accepted当成捕获完成。
图片翻译不使用此文本限额；结果操作保持独立24000字节及既有配置正值检查。

新增26项Swift（8偏好、12路由/边界、4实际原生交互、2渲染）和2张预期中英PNG，
父已审阅完整接线与测试；共用预算直接引用现有协议常量，不另造限制。
4个现有Python文件新增10项真实契约测试；父侧连同bundle库存门槛联合237项/15.617秒通过。
下一源码core最低665，静态预期744原生方法/122PNG；这些数量不是Mac执行证明。
...
上述修正已持久化为`7f54e7e`。首次正常push遇到既有Windows历史测试的WinError5临时文件替换失败
（1846项/96.167秒、同一原因2失败），保留原日志；不改保护或跳hook，原模块16项/3.598秒通过后，
再次正常privacy/full hook1846项/90.593秒通过并推送。重跑通过不代表旧Windows问题已修。
[run35307421335](https://github.com/mclight-ship-it/cc-translate/actions/runs/35307421335)的提前原生日志
暴露本次fixture编辑重复声明了局部`board`，测试编译失败，尚未执行744方法。
现只删除该意外重复声明，全部原断言和生命周期顺序不变；继续新源码验证，不能借用旧绿灯。

`a4ac252`的正常privacy/full hook1846项/87.149秒通过；
[run35307808246](https://github.com/mclight-ship-it/cc-translate/actions/runs/35307808246)
已修复测试编译并真实执行744项（23前置skip，39失败断言/19 unexpected，417.343秒）。
新增20模型/预检方法通过，OCR编辑不再卡在原码点同步等待；94剪贴板方法逐冻结源码全部通过一次，
没有旧线程警告，但不能据此宣布偶发问题永远消失。15失败方法包括13控件定位、1新增等价拼写回归、
1既有中文截图方法；字体本轮17/18，不能借旧18通过覆盖新增断言。
原始字节比较之外，正在为representable保留精确字节值，让SwiftUI也感知规范等价绑定变化，
不重建编辑器；增加相同字节数的组合标记重排回归，并让等待失败报告实际调用行。
中文截图保留原断言，继续收集直接stderr OCR诊断，不猜测其本次失败原因。
提前日志与116/122PNG已收取并逐项核验，仍缺原6图。整run已实际watch退出1；
903便携/16.317秒、232进程/504.096秒、665核心/6.229秒、21后置Foundation/185.599秒通过，
...
`67f280c`正常hook1846项/104.879秒通过。
[run35314198454](https://github.com/mclight-ship-it/cc-translate/actions/runs/35314198454)
提前744项/23前置skip/7失败断言（3 unexpected）/410.658秒：计数与旧中文粘贴说明OCR
现均通过，原断言未变；历史失败恢复也通过。剩历史取消后再应用、2摘要操作，
以及主输入在模型回读完成但实际按钮尚未刷新时过早检查enabled的1条断言。
持久诊断已确认摘要点击后真实控件state变为off，但模型仍idle、保存次数0，不再推断保存已发生。
继续修正队列取出后的正常事件分派，以提供正确的NSApplication.currentEvent；
并让输入用例等待实际按钮就绪。历史设置同时修复同一草稿的原生重提交会清空确认态的问题，
在原模型方法中增加重复提交回归断言，实际改动草稿仍撤销确认；尚待新源码验证原生完整流程。

`87b4be6`正常hook1846项/86.530秒通过。新原生历史模型回归已通过，但8个交互方法
在测试事件队列匹配处提前失败（744项/23skip/16失败断言，8 unexpected，367.554秒），
不是8个产品保存故障；两处OCR和18字号/94剪贴板仍通过。
下一修正不再把自选eventNumber当跨队列身份，而是核对同窗口、类型、位置和时间，
并把取出的真实字段与创建字段一起留在附件；不匹配仍报错，不分派到其他窗口。

`243fd22`实际确认队列把事件编号200000转换为3392，窗口/位置不变且时间仅纳秒舍入；
按真实空间/时间匹配后，摘要2项完整保存/回读/恢复/重开和全部4项输入操作已通过。
本轮744项/23skip/2失败断言（1 unexpected）/516.494秒，仅剩历史“降低并保存”按钮。
历史取消后再次提出确认已通过，确认按钮收到应用级事件后仍未进入原生tracking。
继续明确区分原生按钮自己的cell tracking与SwiftUI checkbox的应用手势分派，
复用同一已取出的事件，不激活其他应用、不重复点击、不改state或模型来冒充操作。

`27bc99f`正常重试hook1846项/87.964秒通过，旧Windows历史AccessDenied仍另列未解决。
本轮744项/23skip/4失败断言（2 unexpected）/588.115秒；摘要和输入全部通过，
历史两个方法已走完英文迭代，但在中文动态“重新读取”/“降低并保存”控件查找时失败。
英文确认按钮已实际进入原生tracking并完成保存/回读/重开，不再是此前的确认动作问题。
动态查找改为有界异步等待，让MainActor有机会完成SwiftUI控件插入和绘制；
...
降低条数显示旧值到新值的确认，取消不发请求。17/600等旧值保留，0/负数/超范围原样显示并允许明确纠正，
不静默钳位。无CLI、关闭历史或翻译正在进行时仍可保存；失败仅提供主动重读，不自动重写或取消翻译。

保存不立即删除历史；下次实际新增时保留最新N条，包含新记录，正在进行的翻译也可能按新值修剪。
提高条数不能恢复已删记录。配置保存/回读不改变同一helper中的历史revision/cursor；
整个helper重开按既有合同换revision，旧cursor失效，磁盘历史内容保持不变。
实际`recorded`终态会把已有列表标为待刷新，保留显示记录/查询/结果，退役旧分页游标与迟到页；
不自动读取、不本地截断，也不因cache/未记录/失败/取消结果刷新历史。

新增21项Swift（12偏好、6历史、2原生交互、1渲染）及2张预期PNG；
共享fixture补真实规范化默认，不删除旧断言。4个现有Python测试文件新增7方法，
验证真实配置/历史字节、修剪时机、原子替换失败、搜索分页及helper重开。
父侧连同bundle库存门槛联合236项/10.893秒通过；下一源码core最低数从648提升至655。
这些未在569bca6执行，不借用该run结果。原生按钮定位与摘要测试一并检查，完整切片仍待新Mac CI。
现已合并待验修复：编辑器先完成真实文档布局再恢复阅读锚点；
摘要/历史交互测试共用typed AppKit可访问性树定位与实际press动作，不再假定SwiftUI必定暴露NSButton子视图。
状态/身份/焦点、取消/保存/回读、可见性和OCR断言保留，缺失或歧义节点仍失败并输出实际节点信息。
未为测试替换产品控件，也未使用模型setter代替交互。合并静态库存718项Swift、预期120PNG；
这些数量不是运行证明，必须由新源码Mac CI实际编译、执行和生成。

`464a9a7`/[run35299872986](https://github.com/mclight-ship-it/cc-translate/actions/runs/35299872986)
已实际退出1：718项Swift/322.563秒、23前置skip，10方法有26个失败断言（9 unexpected）。
历史12偏好/6列表方法及新增7 Python方法逐冻结源码核验通过；3历史UI与4摘要UI仍无法定位控件。
诊断中可见SwiftUI承载按钮，但角色为AXUnknown且没有预期标识；不能据此宣称语义树或点击已经可用。
字体初始长文文档高度的新断言通过，但2项滚动/阅读锚点检查仍失败，先前ensureLayout不是完整修复。
另一个既有来源弹层测试在清空后的SwiftUI更新尚未完成时立即断言移除/关闭，也出现失败。

本run的11 worker/46粘贴/37关联复制各方法仍实际通过、无旧后台promise警告；
893便携/13.043秒、232进程/485.149秒、655核心/5.172秒、后置21 Foundation/175.912秒通过。
UI artifact10529014929仍为116/120PNG，缺2张摘要确认态和2张历史设置图，所有实际PNG的CRC/尺寸/SHA已核验。
未生成App artifact、14/26未运行，推荐包不变。正常push曾因既有Windows旧实现原子替换
PermissionError(13, Access denied)失败1项（1836/97.889秒），同方法1/2.690秒通过，
普通完整重试1836/90.480秒通过才推送；没有绕过hook，也不将其称为Windows旧问题已修复。

继续验证真实呈现的fixture窗口，而非只生成离屏位图；补充未实现typed AX协议的原始节点类型和
编辑器frame/bounds/clip/选择几何诊断。来源清空测试等待实际移除和关闭后再执行原断言，不放宽要求。
CI现在以pipefail保留全部原生build/test退出语义，并在独立包检查之前上传原始unit日志，
避免必须等完整20分钟才能诊断已发生的UI失败；原生发布门禁及全部测试范围不变。
现有bundle/runtime联合72项/14.813秒通过，这些修正与诊断仍需新Mac执行，不预先宣布已解决。

`fc089e6`/[run35301856654](https://github.com/mclight-ship-it/cc-translate/actions/runs/35301856654)
已提前取得实际原生unit日志（artifact10529464351）：718项/355.928秒、23前置skip，
9方法有24个失败断言（9 unexpected）。来源清空方法此次通过，7设置UI及2字体方法仍失败；
这不是完整run通过。UI artifact10529289595实际116PNG，仍缺4张预期设置图。

新几何证据终于区分了测试与生产问题：AppKit的文档frame原点实际为y=-1422；
clip原点-333对应文档内可见位置1089，负clip坐标本身合法。旧阅读锚点把clip坐标当文档内glyph坐标，
又直接用文档坐标滚动clip，导致字号变化后跳到错误段落。现改用`documentVisibleRect`捕获，
恢复时经`view.convert`转换回clip坐标；测试仍要求确实向下滚动、同段落可见和选择/撤销/IME保留，
只是使用正确的文档坐标，另明确断言捕获的锚点坐标。
窗口可见本身未解决SwiftUI语义控件查找。测试树现在同时读取公开的现代与非正式AppKit可访问性接口，
不再只遍历现代接口的承载视图；旧接口的动作须实际被控件声明，保存/回读等效果断言仍保留。
没有伪造控件、直接写模型或使用私有API。新坐标修复与双接口查找仍需自己的Mac执行结果。

`45edbfc`/[run35303254425](https://github.com/mclight-ship-it/cc-translate/actions/runs/35303254425)
的早期日志现已实际验证坐标修复：原2项滚动回归及全部18项字体方法均started/passed各一次。
94项剪贴板方法仍逐项通过、没有旧线程警告。但双接口方案仍未找到SwiftUI语义按钮：
718项/375.654秒、23前置skip，余下7设置UI方法有18个失败断言（9 unexpected）。
116/120PNG的CRC/尺寸/压缩数据/SHA已核验，仍缺4张设置图；不把字体局部通过推广为完整App。
继续定位SwiftUI语义树访问方式，不修改产品控件来迎合测试。fc089e6整run已实际watch退出1，
893便携/12.530秒、232进程/495.038秒、655核心/5.032秒、21后置Foundation/179.619秒通过；
原来源清空方法实际通过一次，App发布仍被原生失败门禁阻止。
...
- [x] 原生请求状态→包内核心/合成provider→展示及真实进程EOF/取消/退出；
  当前同包253进程/21Foundation和原生UI回归见[完整检查点](#native-p4-complete-checkpoint)，物理触发另验。
...
## P1 — 产品所需共享核心已接线；平台实测和既有 Windows 问题另列

Mac 编译/XCTest/原生包内 IPC/Mach-O/HTTPS/SQLite、Codex/Claude链路及P2/P3界面已接线；
当前[完整同包证据](#native-p4-complete-checkpoint)覆盖253进程、744核心和21Foundation。
下面早期子切片中的“尚未接线”限定于它们当时的源码，不代表当前产品仍缺少这些功能。
...
- [x] 产品所需分类/方向/提示词、请求快照、缓存签名与词典结构共享，保留Windows兼容入口；
  后续业务IPC、词典产品及原生请求链已验证，以下保留各子切片最初的边界。
...
- [x] Codex native配置/目录/生命周期及真实版本/预热；本轮已有用户翻译成功反馈。
  ...
  - [x] 当前Codex产品路线的常驻app-server、预热/取消与提交快照已经接通；
    采用真实自有进程监督而非仅在旧kill里补killpg。早期未使用的legacy exec入口不冒充已移植。
- [x] Claude独立生命周期/流式/诊断适配；[后端与原生选择验收](#native-claude-provider-checkpoint)，
  未知认证明确展示，真实账号/模型另由用户操作。
- [x] POSIX本App自有进程组取消/回收及原生helper接线；warm不创建付费turn、请求不自动重试；
  不保证不可捕获SIGKILL/崩溃后的主动逃离组后代清理。
  - [x] 原生显式版本探针先接入自有组监督；fake CLI 正常/取消/超时/输出超限/后代回归。
- [x] 共享核心直接import、Windows实际消费者及Mac同源差分回归；
  当前正常1973项本机回归通过，旧Windows原子替换偶发拒绝仍单列，不能以重试消除该记录。
...
- 原生登录项使用`SMAppService.mainApp`的实际状态，不用本地偏好伪装已开启。
  ...
  实际NSButton存在，OCR未读出完整标题；另11项新增方法及全部旧方法通过，140张图库存完整。
  不能把图齐全或旧包绿色当本轮通过。测试改用公共AX标识/名称/屏幕几何关联真实NSButton，
  仍发送原生鼠标事件、检查同一开关及操作次数，不直调业务动作、不改生产代码或删断言；修复待新源码验证。
  首轮完整run已实际watch exit1，仅最终原生成功门禁失败，consumer跳过且没有App发布。
  AX修正`78c4299` / run`35365838458`也已实际编译（39.29s），仍是同一交互方法失败：
  `remove-pending-login-item`的semanticMatches=0，844项/23前置skip/2失败/551.962s，140PNG完整。
  因CI的SwiftUI语义树不可取得，撤掉这条测试定位路径；下一修正复用既有真实双分辨率位图分配，
  仅新登录按钮使用作者UI文字的语言校正/实际候选读数，保留精确标题几何和真实NSButton鼠标事件；
  不注入识别提示、不替换错字，也不把未执行的中文交互或此源码当通过。
  `78c4299`完整watch已退出1，仅最终原生成功门禁失败，consumer跳过且未发布App。
...
新增13项便携契约，bundle/runtime联合93项/16.796s通过；源码18d6d6d / run35379767359
已实际Swift编译41.26s、846原生项/23构包前可选skip/零失败/708.441s、140PNG完整，
新增真实SwiftPM框架加载/不启动更新会话方法通过。该run已实际watch exit0，attempt1全3jobs/42steps成功；
portable1003/22.351s、producer253process/567.592s及744core/7.746s通过，
同一App在15/14/26各253process、744core、21后置Foundation通过，零后置skip。
完整App artifact10562218159已独立核对：实际0.1.0/build147，
20,638,833字节、SHA256 `a788508215673c7095dc91a8031577cfe6d95a9621a4110be3ab076b41b9040c`，
788库存/84资源/55源码路径（54唯一）/11真实Mach-O/19Python运行时许可及Sparkle完整许可。
厂商94项字节、权限、链接逐项匹配独立官方归档；三系统HTTPS证书、SQLite、取消/EOF、
bundle不可变及临时清理均通过。只下载一次，最终复核大小/hash后已仅删除确切App ZIP。
会话采集器曾错误要求非verbose便携日志提供逐名结果，修正后复用原下载审计通过；
该轮13项新便携方法只有AST定义、明确模块执行及整体无skip的OK证据，不冒称逐名日志通过。
正常privacy/2文件编译/完整hook1946项/104.289s通过。该框架检查点当时未替换4dfb04a/build146推荐包。
本步不启动更新检查、没有feed/密钥配置，不发布Release；检查更新界面、签名升级和重启保护继续实施。

<a id="native-update-ui-checkpoint"></a>

### 已核验：原生检查更新入口；签名升级继续实施

源码`041df148a6b4a3c45820ea9ae22f8f6953d5cb0b` /
[run35387156483](https://github.com/mclight-ship-it/cc-translate/actions/runs/35387156483)
已实际watch exit0，attempt1全3jobs/42steps成功，当前推荐升级为0.1.0/build150。
Swift真实编译32.88s；861原生项（838通过、23既定构包前可选skip、零失败）/610.011s，
新增15方法全部通过：英中真实按钮点击/禁用/恢复/精确次数/不启动helper、真实菜单、
默认不启动更新器、错误/退出状态及三张更新图的正文/背景/主题回归均通过。
[143张PNG](https://github.com/mclight-ship-it/cc-translate/actions/runs/35387156483/artifacts/10564982505)
库存/CRC/尺寸/IDAT完整，三个新图已实际查看。

producer1004portable/18.536s、253process/520.496s、744core/6.053s通过；
便携新增13项框架及1项更新默认契约均有逐名一次passed，按便携/随包core/process各自步骤核对，
不将不同runner的有意重复执行误判为重复，也不回填旧非verbose日志。
同一App在15.7.9、14.8.9、26.6.2各253process/744core/21后置Foundation通过，零后置skip；
About实际0.1.0/build150、Vision4、主动粘贴46、关联复制读取7及真实只读worker11继续通过。
三系统HTTPS证书、SQLite、取消/EOF、bundle不可变、临时清理均已读回通过。

[完整App artifact10565210962](https://github.com/mclight-ship-it/cc-translate/actions/runs/35387156483/artifacts/10565210962)
外层20,401,669字节，内层20,660,910字节；
SHA256 `072b11ab5dfa4b33a7914ec1934557e87f40c3c2f5d0e6b5f14e17226805a477`，
tree `83a4b89c528af38a6b92a2a306d2d4e8a064e31bb851fc10ae5e7cfc8673c9ec`。
独立核对788库存/84资源/55源码路径（54唯一）/11真实Mach-O/19Python运行时许可及Sparkle完整许可，
厂商94项字节/权限/链接逐项匹配固定官方归档，C桥排除构建元数据后的hash与已验基线相同。
实际Info/清单构建150及原bundle身份匹配；四项SU默认值精确核对，无feed/public key。
14/26小报告分别为10565242467、10566231633；完整App只下载一次，最终重核大小/hash后已仅删除确切内层ZIP。
App到期2026-09-25T20:04:55Z；没有Release/feed/正式密钥或真实签名升级验证。

正常push首次复现旧Windows配置原子替换WinError5：1947项/97.327s中，同一未改loader方法产生43个子案例失败。
原日志保留；该方法单项/2.546s通过后，完整正常privacy/1947项/95.258s通过并推送。
未改Windows代码、未绕hook、未关闭防护；重试通过不表示旧稳定性问题已修。
下面保留此前失败/修复过程，不把9fa/11的失败或旧147框架绿灯代替本源码验证。

本源码已接入菜单栏/应用菜单“检查更新”、独立原生更新窗口及设置区域；
已有渠道时，明确点击才懒创建并启动Sparkle，使用其标准下载/安装界面及实际可检查/会话状态。
没有已发布渠道的开发包如实显示说明，单独点击可打开已验证下载页面；不谎报“已是最新版”，
不让用户填feed/密钥，不启动翻译helper或改CLI/账号，不通过自动开浏览器掩盖失败。
默认关闭自动检查、自动下载及系统画像；设置恢复默认不改变更新器偏好，
退出时拒绝新的检查/浏览器动作，保留应用既有退出排空处理；
Sparkle真实安装重启/取消是否完整保留该处理，仍待端到端验证。
源码9fa9661 / run35383335452已真实编译55.80s，861项/588.899s中23构包前可选skip，
两项新方法发生5个失败断言（1unexpected）：英文检查按钮未被OCR完整定位，
以及更新窗口浅色正文/错误和非空色彩断言失败。新增15方法其余13项通过，143张图虽完整但不是绿灯。
原失败日志和图保留；三个新图实际背景像素alpha均为0。
现按已有About窗口补系统背景，增加不透明度和明暗色断言，原按钮交互和正文断言保持，
修正仍需自己的Mac验证。便携CI同时启用原runner的verbose，后续可检查实际逐名结果，不回填旧日志。
新增1项默认值便携契约已纳入原有CI选择器；修正后与bundle/runtime合计94项/17.905s通过。
9fa完整run已实际watch exit1：最终原生门禁拒绝归档/发布，runtime被跳过，只有日志和图片产物，没有下载App。
背景修正11ffa24 / run35385604418已真实编译25.36s，861项/464.825s中仅剩同一英文按钮定位方法
2个失败断言（1unexpected）；23构包前可选skip不变。三个更新渲染及新增背景断言全过，
实际图片背景alpha255，浅色RGB236、暗色RGB50；143PNG完整（artifact10563967239），但整轮仍未通过。
当前进一步只修测试定位：使用完整操作词而非末尾省略号点数，不改产品文案、共享OCR或真实点击方式；
仍要求唯一可见NSButton、检查后禁用/结束后启用、精确服务/浏览器次数及不启动helper，另断言检查不打开浏览器。
背景源码的正常privacy及1947项/104.893s通过；定位修正还需自己的Mac执行，不沿用旧包绿灯。
...
不按配置里的路径追删CLI、外部词典、共享账号或Node，不递归清空Library；重定向的父目录
在任何移动前被拒绝，目录内链接随目录移动而不遍历外部目标。

先确认自身登录项移除，再复用已有退出路径排空主/诊断helper及图片清理；更新会话进行中
不启动卸载。数据操作开始后禁止迟到窗口/设置事件重写原生偏好。失败记录具体阶段和已完成
项目、停止后续删除并明确告知部分结果，不假装回滚；已移入废纸篓的文件可恢复，偏好清除
不可恢复。普通退出不卸载，关闭确认不触发任何文件移动。
...
使用独立的Sparkle public user-driver，目标是当前完整App的唯一标识临时副本，而非正式应用：
临时CryptoKit Ed25519密钥仅在私有临时目录中以0600权限使用，不进入Keychain或构建产物；
loopback feed/HTTP例外仅写临时副本和外部driver，不改产品默认feed/public key/HTTPS约束。
hostBundle与applicationBundle均指向被测副本，外部driver自身不替代被测应用退出/重启。

验证有效签名1→2安装、旧PID退出/新PID启动、坏签名、错误公钥、下载与安装前取消、
下载失败，拒绝或取消后App字节不变；通过包内Python实际配置owner前后读回配置，
并检查自有数据哨兵及原生偏好。副本来自同一源码，所以不是两版业务迁移或真人TCC保持证明。
...
布局源码`edd8db82746f1fad7d9878eb209a30b2b16a55a6` /
[run35458284459](https://github.com/mclight-ship-it/cc-translate/actions/runs/35458284459)
已实际跨过签名/严格深度校验，并完成有效签名安装：磁盘build2，原PID20408退出、
新PID20422启动、身份相同、优雅清理完成。Swift36.46秒、890原生项/23既定skip/
零失败、595.255秒，151PNG完整。
但整体仍exit1：配置复核脚本将`font_size`与字符串`"16"`比较，而共享配置读回规范化为整数；
最初seed只检查自己的内存字典，没重新读取保存值，漏掉了这一fixture错误。
当前修正改为整数seed并立即经真实owner读回，之后仍严格比较前后配置值和文件SHA；
增加跨seed/read及字号改变必须失败的便携回归，不改产品规范化行为。
原始install报告另显示Launch Services的新进程未必先到通知回调；
最终枚举已确认归属的实例也写PID ledger，再终止，所有终态PID必须出现在ledger。
坏签名/错公钥/取消/下载失败仍未执行，不把这个局部正向安装当六场景全部通过；
临时密钥/目录清理true，无完整App发布/下载或三系统消费者证据。

读回复核源码`232b0e77c07a977d995281a41d7ad8aaf0c89c5e` /
[run35459318418](https://github.com/mclight-ship-it/cc-translate/actions/runs/35459318418)
的正向安装场景已完整通过：原PID67893退出、新PID67923启动且两者都在ledger；
配置前后字号16、marker、文件SHA完全一致，自有数据哨兵/原生偏好保留，优雅清理。
但整run仍exit1：坏签名真实被Sparkle拒绝，原App仍build1/原PID存活，
错误链实际为`4005 -> 4005 -> 3002`，末项为`EdDSA signature does not match.`；
测试误将这个EdDSA校验路径的期望写成了3001。固定Sparkle2.10.0头文件及实现确认
3002是该校验错误，并非任意失败即可通过。
修正要求Sparkle域、3002及明确EdDSA不匹配说明；继续拒绝传输、配置、普通bundle校验和
仅安装层4005，删除只看顶层错误码的误导性signature_error字段。
现在每个场景完成自己的清理后可汇总断言/子命令失败并继续采集其他场景，最后任何失败仍退出1；
未知进程清理仍立即终止并保留fixture，新增便携回归覆盖两类行为。
本轮Swift38.08秒、890原生项/23前置skip/零失败、655.245秒，151PNG完整；
没有完整App或消费者绿灯，不把正向一项冒充六项全部通过。

源码`3f8628db58e3903786f2577640ec91b075bdb825` /
[run35460404502](https://github.com/mclight-ship-it/cc-translate/actions/runs/35460404502)
进一步实际通过正向安装和坏签名两项，包括完整配置SHA/偏好/自有数据保留，
坏签名后的App树SHA前后相同。但仍exit1：旧临时私钥文件`wrong-key`与第三场景目录同名，
创建目录时报FileExistsError，其余四项尚未运行。现把两把临时密钥隔离到私有keys目录，
新增实际创建全部场景目录/两份合成文件的回归；不是更改签名或放行失败。
本轮Swift32.25秒、890原生项/23可选skip/零失败、574.926秒、151PNG完整。

同步补充fixture清理边界：不可读/损坏/缺字段ledger、ps失败/超时/无法确认PID、
未记录最终清理状态均作为未知清理硬停止并保留目录，防止未观察到重启进程时误删其App。
这些不是用户使用前置，也不修改正常翻译/更新交互。
CI仅把既有构包和签名fixture提前到完整原生测试前，让短的安装测试先报告失败；
...
- [x] 免费开发包完整性/资源/归档检查及同包三系统验证；[当前完整检查点](#native-p4-complete-checkpoint)。
- [ ] 最小权限和干净用户Gatekeeper首开集中验收；不全局关闭保护。
- [ ] 可选付费增强（未选择、未通过）：Developer ID、公证/stapling；不得作为购买要求。
<a id="native-p4-complete-checkpoint"></a>

### P4 自动化闭环基础检查点（build161，已由165继承）

源码`2195dfc60d4dbc3b00535ab597076186e83631e4` /
[run35464063349](https://github.com/mclight-ship-it/cc-translate/actions/runs/35464063349)
已实际watch退出0，3jobs/44steps全部成功。Swift编译21.91秒，890原生项
（867通过、23构包前可选skip、零失败），151PNG完整；13项卸载回归通过。
同一字节App在15.7.9/14.8.9/26.6.2分别通过253进程、744核心及21后置Foundation；
各系统46项主动粘贴零skip/零失败，包含此前macOS26失败的空RTF方法。
构包前的发布就绪修正只等待AppKit元数据，不预读正文/条目、不重复发布或放宽生产换主拒绝。

全部六个真实Sparkle场景通过：安装重启、坏签名、错密钥、下载取消、安装前取消和HTTP503。
正向安装旧PID8788退出、新PID8802运行build2；配置前后SHA一致，
偏好/自有数据哨兵保留，其他场景保持build1及原App字节。临时密钥、副本、自有数据和进程均清理。
独立原App树`4b91e2c3bfc76515b5ccd3239e6652af3f72a0bbc1e2c0a3e64cbcb3b6bafa70`
与fixture及两consumer一致；788库存、84资源hash、55源码路径（54唯一）已核对。
这证明一次性副本、临时Ed25519密钥和同源码build1→2流程，不证明正式feed、两版业务迁移或真实TCC保持。

[完整App下载](https://github.com/mclight-ship-it/cc-translate/actions/runs/35464063349/artifacts/10590564965)；
内层`CCTranslateMac-P0.zip`为20,691,347 bytes，
SHA-256 `eaf1563c53fa70cca18ec61b714572866b6a1e03a892262a30abf3717b5480c8`，
保留至2026-09-26T19:44:50Z。本地仅下载一次，最终重hash后已删除确切内层ZIP，保留报告和截图。
本轮原生collector曾误传PNG基线目录；Mac测试本身通过，已用正确JSON基线验证同一批151PNG，
没有重下载图片或重跑Mac。此前155–160真实失败记录保留，不把它们改写为成功。

- [x] SMAppService 实际状态与原生设置、明确开关/批准入口及自动化；真实登录启动另验。
- [x] 独立Mac版本/开发构建号、清单绑定及真实同包About验证。
- [x] 固定Sparkle框架/完整许可与原生检查更新入口；启动默认不检查，无渠道时明确手动下载。
- [ ] 独立Mac更新资产与更新签名；无已发布feed或私钥，不以入口测试代替签名安装。
- [x] 一次性副本/临时密钥的六个真实签名更新场景、配置/数据保留及实际重启；见上述完整检查点。
- [ ] N→N+1 更新保护数据/权限；失败/取消/重启；不在 bundle git pull。
- [x] 卸载可选清理自身数据，不删除共享 CLI/账号/Node；排空交互及同包三系统已通过。
- [ ] 应用自身许可确认；Python/依赖/词典/更新框架完整许可齐全。

<a id="native-performance-checkpoint"></a>

## P5 — 自动化性能基线已验证，设备和长期验收另列
- [ ] 第一轮平台探针验收（应在 P0 尽早组织）。
- [ ] 第二轮完整候选：TextEdit/Safari/Chrome/PDF/VS Code/Terminal、IME、多屏全屏。
- [x] 包内固定样本分类/常驻配置IPC/词典查询格式化P95及原始样本，同包三系统验证见下。
- [ ] 完整GUI触发到首屏及真实模型首字/完成A/B；不以离屏或无模型测量代替。
- [x] 真实隔离App的短时空闲CPU/内存原始计数及正确时间基换算；不冒充交互长稳。
- [ ] 用户设备长稳/休眠、真实权限拒绝及并发系统交互；既有UI时序/provider故障自动化不替代此项。
- [ ] 预授权 CI 与首次用户 TCC 区分；未解决高优故障为零。

固定样本的无模型测量已接入既有CI并完成自身Mac执行，不借用旧版本耗时。
包内Python执行8类合成文本分类和5个固定真实词条的查询/纯文本格式化；
常驻配置helper使用现有只读config_load往返，不增加生产ping操作或调用模型。
每项分开记录首次调用、3次不计入样本的预热、100次原始样本、nearest-rank P95及最大值。
分类/往返/查询格式化分别对照2/5/10ms目标；未达到目标如实标为measured_miss，
不新增用户使用门槛，不把测试通过等同于性能达标。
首次调用在模块导入之后，不宣称清空OS缓存；另记helper进程启动到ready和词典打开/固定SHA核验时间。
IPC包含配置处理、JSON与接收队列，不冒充纯管道传输。各系统执行同字节App并保留独立报告。
既有SwiftUI/AppKit离屏首屏10样本仍单列，不冒充完整打包GUI上屏；
空闲CPU/内存、长稳/休眠、物理按键与模型A/B仍未由该测量覆盖。
首源码`380fb4f31e267e84694755265fe30dd2d53775d1` /
[run35464856466](https://github.com/mclight-ship-it/cc-translate/actions/runs/35464856466)
在1025项便携测试中有1项新断言失败：macOS临时目录`/var`解析为`/private/var`，
实际选中的随包解释器正确，测试却比较未规范化的路径。当前修正预期的canonical路径，
并让同一测试通过带`..`的App路径验证规范化。未修改生产路径或性能测量行为；
本轮未构包/执行Mac性能，不报告耗时达标。正常提交前103项通过、完整hook1968项通过的
Windows结果不替代这次真实Mac失败。

修正源码`70bb6e6ed74010d057d83a9df93f50c9d63f4fed` /
[run35465025735](https://github.com/mclight-ship-it/cc-translate/actions/runs/35465025735)
已完成watch0、3jobs/44steps及独立完整App审计：890原生项（23构包前可选skip、零失败）、
151PNG，以及同包三系统各253进程/744核心/21Foundation。每项100个原始样本独立重算，
全部分类/词典/常驻IPC的P95目标均满足。下表取各分类或词条自己的P95的最大值，不混池计算：

| macOS | 8类分类最大P95 / 峰值 ms | 5词条最大P95 / 峰值 ms | 常驻IPC P95 / 峰值 ms |
|---|---|---|---|
| 15.7.9 | 1.492959 / 1.793334 | 0.093875 / 0.105459 | 0.610375 / 0.688000 |
| 14.8.9 | 1.484292 / 1.502125 | 0.098834 / 0.134292 | 0.980708 / 1.162667 |
| 26.6.2 | 1.818791 / 2.521709 | 0.136792 / 0.246625 | 1.370417 / 2.307375 |

macOS26长文峰值2.522ms未隐去；目标是P95而不是每次最大值。
helper启动到ready分别204.315/295.100/274.474ms，词典打开加固定SHA检查33.689/79.480/91.093ms，
不计入warmP95，也不当完整GUI冷首屏已达到150ms。三系统helper退出/临时home清理/App不可变均通过。
内层20,691,347 bytes，SHA`6423ef751dae881da94d1de6b68de4db2ff573c65e01384f643ffeaf38d32cc0`，
只下载一次并最终重hash后删除；本轮未要求用户为了测试工具变更再次更换可用的build161。

继续补充空闲资源基线：复用已隔离的真实App更新fixture，仅正向安装场景在发起更新检查前
静置5秒，再用libproc对该App PID及其当前后代每秒采样，31个快照覆盖至少30秒。
记录原始CPU累计计数、RSS、physical footprint及进程启动身份；按实际单调时间计算单核百分比，
另列全区间CPU、各区间P95/最大值和内存P95/最大值，不混同CPU均值与P95。
测量发生在测试驱动之外的真实App进程，驱动自身不计入；不读取其他用户应用或调用模型。
进程集合/身份变化、计数回退、缺少快照会明确报告测量失败；高CPU/内存本身不设使用门槛。
沿用原有自有PID退出和临时目录清理；安装场景仅增加35秒采样预算，其余场景不延迟。
首轮`b2663aa`/build164已真实编译并通过六个更新场景，采到31快照/31.850秒和稳定1个App进程。
复核API单位时发现CPU累计字段是Mach ticks，不能直接当ns；首版CPU统计撤回，不把绿色测试当单位正确。
已改为保留原始ticks和实测mach_timebase_info分子/分母，再换算CPU时间，新增非1:1时间基回归。
本轮内存原始字节仍保留，但不拿旧CPU数值猜倍率修正；其CPU审计已明确拒绝，完整App ZIP核验后清理。
这不是用户设备实测、长稳或休眠/唤醒证明，也不改变App产品行为。

### 当前最终自动化检查点：build165

修正源码`e6ab5dc87799ef45a7b5ce774847167a8264f597` /
[run35466603008](https://github.com/mclight-ship-it/cc-translate/actions/runs/35466603008)
已实际watch0、独立审计0、3jobs/44steps成功；本机正常privacy/编译/1973项回归通过。
真实Mac便携1030项、890原生项（867通过、23构包前可选skip、零失败）及151PNG通过；
同包15.7.9/14.8.9/26.6.2各253进程/744核心/21后置Foundation、46主动粘贴零skip/零失败，
六个真实签名场景再通过。正向安装PID3759→4521，配置/偏好/数据保留，临时密钥/目录/进程清理通过。
788库存、84资源hash、55源码路径（54唯一）、版本0.1.0/build165和原App树
`76660a2d42f1c4e23fe667dfb183a3db9aac274e5cf6014608159e16c419e79f`已独立核对。

最新同包的实际测量如下，保留相对build163的运行差异，不修改阈值或无修改重跑来挑选好结果：

| macOS | 分类最大P95 / 峰值 ms | 词典最大P95 / 峰值 ms | IPC P95 / 峰值 ms | 分类2ms目标 |
|---|---|---|---|---|
| 15.7.9 | 2.835292 / 4.461708 | 0.150625 / 0.297125 | 1.244625 / 1.421875 | 长文case measured_miss |
| 14.8.9 | 2.019917 / 2.080125 | 0.136542 / 0.151292 | 0.738292 / 0.831209 | 长文case measured_miss |
| 26.6.2 | 1.642209 / 1.729500 | 0.104916 / 0.145791 | 0.632958 / 0.654583 | met |

词典/IPC全部目标满足；两个系统的约6KB长文分类P95未达到2ms内部目标，已如实记录，
不将正确性通过写成全部性能达标，也不因此增加使用门槛或改变分类规则。
helper启动到ready为263.354/274.866/236.270ms，词典打开+SHA检查38.057/64.895/70.189ms，仍与warm分开。
所有helper退出/临时home清理/App不可变均通过。

空闲测量这次实际读取Mach时间基125/3；31快照覆盖31.272秒，稳定1个App进程。
独立重算单核CPU百分比P95为0.074400%、峰值0.128849%、全区间0.011585%；
RSS P95为48,726,016 bytes、峰值48,906,240 bytes（46.64MiB）；
physical footprint P95为9,602,752 bytes、峰值9,750,208 bytes。
这些只是producer无输入短时空闲数据，不代表打开所有窗口后的负载、用户机器或长期运行。

[完整App下载](https://github.com/mclight-ship-it/cc-translate/actions/runs/35466603008/artifacts/10591314230)；
内层`CCTranslateMac-P0.zip`为20,691,349 bytes，
SHA-256 `e58eeb5882da6530ef819b90954add665e7a12ec0e31f88b55eb9998db9b29e6`，
保留至2026-09-26T20:38:14Z。本地仅下载一次，最终重hash后删除确切ZIP。
原build161功能可继续使用；165主要补充测量/验证，不要求已测通用户重新配置或反复下载。

## P6 — 等待 P5 和另行发布授权
- [x] 开发包安装说明、原生截图、已验CLI/OS/CPU范围、已知局限与诊断；
  [当前下载与步骤](MACOS_DEVELOPMENT.md#native-translation-user-check)，不把开发包说明当正式发行签收。
