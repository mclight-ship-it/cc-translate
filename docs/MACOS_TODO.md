...
最新推荐包现为[2195dfc：Mac 0.1.0/build161，包含卸载排空交互及已验证的可签名运行时布局](#native-p4-complete-checkpoint)，
已完成完整App同包三系统及六个临时签名升级场景；继续P5测量与平台验收，不把移植整体标为完成。
...
固定样本的无模型测量已接入既有CI，等待其自身Mac执行结果，不借用旧版本耗时。
...
本轮内存原始字节仍保留，但不拿旧CPU数值猜倍率修正；等待新源码自身Mac验证。
