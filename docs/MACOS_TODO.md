...
用户授权逐项实现性能调查中的优化；推荐安装包升级为 **0.1.0/build206**。
...
再次对照 Windows 与 Mac 源码后确认：Mac build206 **已经有** Codex 无内容预热、
  ...
  仍有以下覆盖差异，属于可验证的下一步机会，不是本轮已经测出的速度收益：
...
尤其是 Mac 双击回调当前只尝试 `resumePrewarm()`；若 helper 已因休眠断开，
  ...
  不能简单在每个按键上另起 CLI。以上差异仅做源码核实，尚未修改产品代码或计为工程验收通过。
...
最新推荐包现为[8007747：Mac 0.1.0/build206，翻译准备与首字提速](#native-translation-latency)，
