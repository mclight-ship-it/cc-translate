...
本轮修正 build174 实测反馈；在新源码通过 Mac 验证前，推荐下载仍为 build174。

- 菜单动作统一为纯文字，保留开关勾选；移除翻译入口的 Cmd+N。
- Dock 仅在有产品窗口打开时出现；最小化或隐藏不是关闭，关闭最后一个窗口才回到菜单栏模式。
- 排查普通 Cmd+C 可以复制、双击复制却取不到文字的浏览器/PDF 兼容问题。
  保留手势内的新复制边界，不读取手势前的旧内容，不改写用户剪贴板。
...
原生编译、窗口/设置交互、截图自动提交与取消、复制兼容、赞助资源及同包验证尚在进行；
不能将下方 build174 的绿色记录当成本轮代码已经通过。
