...
最新推荐包现为[2fa45ee：Mac 0.1.0/build183，快捷翻译减步骤](#native-quick-translation-feedback)，
已完成完整App同包三系统及六个临时签名升级场景；设备集中验收和正式发行仍另列。

<a id="native-pearl-ui"></a>

### Pearl 原生界面接入（2026-09-22，验证中）
...
- 快速结果仍是独立浮窗；截图入口仍直接开始选区并自动翻译，不增加确认页。
- 新本地词典页面复用真实 helper 的离线查询及下载管理；未命中、未安装或出错时明确显示状态，
  不自动转为 AI 请求，也不改写翻译草稿或历史。
- 原生构建、回归与新截图验证完成前，仍以 build183 为已验证安装包。
  不用 HTML 原型截图代替 App 渲染结果，不将自动化当作实机 VoiceOver/TCC 验收。
