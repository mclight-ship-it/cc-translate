...
## 翻译延迟优化（2026-09-24，实施中）

用户授权逐项实现性能调查中的优化；上一份已验证安装包仍为 build202。
不调用用户真实账号/模型，不以无模型探针冒充真实翻译速度。

- [ ] Codex 成功版本检查按可执行文件身份缓存；保留取消、升级失效及进程组清理。
- [ ] 当前服务单进程、十分钟闲置回收；将无模型预热接入明确翻译意图。
- [ ] Claude 只保留一个未使用的预热进程，匹配模型与提示词，使用后补充；不自动重放。
- [ ] 首段流式文字立即更新，后续保留合并刷新；明显非词条输入省去词典往返。
- [ ] 最近二十次请求的内存计时，只保留阶段耗时、固定分类和命中标记，不包含原文/译文/账号。
- [ ] 验证快捷键取词、首次/连续/闲置后请求、取消/退出/休眠、缓存和两端既有行为。
- [ ] 免费 Mac CI、同包验证、官方 CLI 无模型冷/热准备计时及新包交付。
...
最新推荐包现为[7953587：Mac 0.1.0/build202，单工作区与快速输入修复](#native-usability-repair)，
