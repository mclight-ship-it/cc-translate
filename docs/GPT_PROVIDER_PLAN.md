...
- 最新真实探针确认 23 个 delta 与最终 `item/completed` 文本一致，真实中断在首个
  delta 后返回 `cancelled`。
- Codex 设置只提供“智能路由（极速）”和“自动选择（优质）”。mini 保留为智能路由的
  内部分支，不再作为独立用户选项。
...
- 合并此前长文 A/B 后，400 字以上流式 mini 的首字中位为 5.08 s，流式极速 Auto 为
  5.52 s。因此“智能路由（极速）”现在对所有文字使用 app-server：400 字以下及词典
  使用极速 Auto，400 字以上使用 mini。stable 仅保留为显示任何输出前的安全回退。
...
智能路由为默认，并在内部按文本长度选择极速 Auto 或 `gpt-5.4-mini`。质量 Auto 保留给
