...
- 最新真实探针确认 23 个 delta 与最终 `item/completed` 文本一致，真实中断在首个
  delta 后返回 `cancelled`。
- 应用启动时会立即异步预热当前选择的 GPT 智能路由；预热只完成 initialize 和
  Defender hook preflight，不创建 thread/turn，也不读取或发送剪贴板内容。首次
  Ctrl+C 仍保留幂等预热兜底。
- 长文“摘要 + 全文”对 Codex 使用独立的 189 字符任务指令，Claude 继续使用原有完整
  prompt。该候选从 1352 字符压缩约 86%，追加 A/B 中摘要结构质量 4/4 通过；相对前一版
  249 字符候选的完成时间配对中位快约 0.75 秒，但首字基本持平。其他任务因复测未显示
  稳定收益，保持原 prompt。
- Codex 设置只提供“智能路由（极速）”和“自动选择（优质）”。极速模式对全部文字使用
  Auto 的低推理、低 verbosity 配置；mini 不再作为用户选项或内部长度分支。
...
- Auto 使用 `none` reasoning + low verbosity 的两次长文首字中位约提前 1 s，但总耗时
  没有稳定下降；小样本不足以改变默认配置。
- 随后完成 27 次固定随机顺序 A/B（短文、长文、列表与代码各 3 次）。`auto-fast`
  在三个场景的首结果中位均比质量 Auto 快约 10–14%，整体首结果中位从 5.60 s 降至
  4.75 s，总完成中位基本持平（7.09 s 对 7.10 s）；27 次均成功，三种候选的质量检查
  均为 9/9。该配置最初作为显式选项上线，后续完成智能路由验证后成为 Codex 默认。
- `gpt-5.4-mini` 本轮只在短文更快、长文反而更慢，因此设置标签改为“轻量”，不再承诺
  它在所有长度上都是快速路径。
- 为验证短文本是否也应流式，追加 104 次固定随机顺序实测，覆盖 2 字 CJK 词、英文单词、
  双词术语、句子及约 120/220/320/420/650/1073 字符文本。比较 stable/app-server 与
  mini/极速 Auto，使用 stable 完成时间和 app-server 首字时间作为统一的用户可见指标。
  104/104 调用及质量检查均通过。
- 词典聚合中位：流式极速 Auto 4.51 s，流式 mini 4.82 s，stable mini 6.90 s；
  400 字以下普通文本聚合中位：流式极速 Auto 4.45 s，流式 mini 5.77 s，
  stable mini 6.39 s。实测证明短文和词典也应流式。
- 当时合并长文小样本后，400 字以上流式 mini 的首字中位为 5.08 s，流式极速 Auto 为
  5.52 s，因此曾临时上线 400 字以上 mini 分支；后续更大规模复测已取代该结论。
- 智能路由成为 Codex 默认；质量 Auto 保留为第二个显式选项。原先保存的显式 mini
...
- 常驻路径上线后追加 mini 推理档位、结构化文本和 420/550/650/750 字边界复测。最终汇总
  32 组可配对长文请求：极速 Auto 首字和完成时间均赢 22/32，中位分别快 0.84 s 和
  1.00 s；极速 Auto 首字中位 4.52 s、完成中位 7.11 s，mini 分别为 4.98 s 和 7.74 s。
  新增的 51 次调用全部成功，所选 Auto/mini 候选的完整性与结构检查全部通过。
- 因此智能路由不再按 400 字切换 mini，所有文字统一使用极速 Auto；这也让运行时只需维护
...
- 预热进程实测约 38.4 MB 工作集、14.7 MB 私有内存。普通复制不会同步等待预热；
  非默认 profile 的仅预热进程 30 秒自动清理，成功翻译后使用常规 5 分钟复用窗口。
- 精简 Codex 请求外壳的 48 次交错 A/B 中，候选方案首字赢 14/24，对内中位快 550 ms，
  提示词中位减少 253 字符；发现一次代码围栏丢失后增加按需代码保护约束。补强版结构化
  复测 16/16 质量通过，首字赢 5/8，对内中位快 862 ms、完成快 604 ms。该版本作为
  `codex-format-v4` 上线，避免旧缓存掩盖新输出契约。
- `auto-fast` 的 app-server 在应用存活期间不再按空闲时间回收，避免长时间未使用后的冷
  启动；其他 profile 仍保持 5 分钟空闲回收。正常退出、更新和卸载继续统一关闭 provider。
...
智能路由为默认，并对全部文字使用极速 Auto。质量 Auto 保留给
更看重译文质量的用户。Codex 可用模型仍会受 ChatGPT 套餐、企业策略和 CLI 版本影响；
模型不可用时显示明确错误，不静默切回 Auto 或 Claude。
