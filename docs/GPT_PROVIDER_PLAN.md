...
- 随后完成 27 次固定随机顺序 A/B（短文、长文、列表与代码各 3 次）。`auto-fast`
  ...
  均为 9/9。该配置以“Auto（极速首屏）”作为显式选项上线，不替换默认质量 Auto。
...
- 合并此前长文 A/B 后，400 字以上流式 mini 的首字中位为 5.08 s，流式极速 Auto 为
  5.52 s。因此“极速模式（智能路由）”现在对所有文字使用 app-server：400 字以下及词典
  使用极速 Auto，400 字以上使用 mini。stable 仅保留为显示任何输出前的安全回退。
- 默认质量 Auto、显式 mini 和流式 Beta 开关的既有行为不变；智能路由只在用户选择
  `auto-fast` 时生效。截图继续使用 Auto 极速参数，不应用未经验证的文字长度路由。
