...
- **自动更新**：本身即 `git clone` 部署，可从 GitHub 检查并 `git pull` 更新后自动重启；支持夜间自动更新（设置可开关，默认凌晨 3 点）与手动「检查更新」；更新后会先编译 + 跑测试校验，失败自动回滚，绝不会更新成起不来的状态；重启完成后弹出托盘气泡确认「已更新并重启」（因为新进程的托盘图标可能被 Windows 收进溢出区，用气泡给出可见反馈）
