...
- **富文本排版**：结果弹窗支持轻量 Markdown（行内代码、代码块、加粗、斜体、标题、列表、链接），并像代码编辑器一样对代码分色显示；代码块可选 Pygments 语法高亮（未安装自动降级为单色），复制出的仍是纯文本
...
- **自动更新**：app 本身即 `git clone` 部署，可从 GitHub 检查并 `git pull` 更新后重启。设置里的「检查更新」只检查不改动，发现新版才显示「更新并重启」按钮由你决定；「夜间自动更新」开关（默认开启）则在后台静默完成。更新前先编译 + 跑测试校验，失败自动回滚，绝不会更新成起不来的状态
- 可设开机自启
