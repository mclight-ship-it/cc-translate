...
- 测试位于 `tests/`，只覆盖纯函数（分类、词典判定、markdown-lite 解析、
  代码块高亮降级），不依赖 GUI / 剪贴板 / 网络 / Claude CLI。
...
3. 跑 `python -m unittest discover -s tests`，确认全绿。
4. 用个人账号 `mclight-ship-it` 提交并推送到 `origin master`
   （提交信息末尾加 `Co-authored-by: Copilot <...>`）。
5. pre-push 钩子会再兜底跑一次测试；被拦截就说明有回归，修好再推。
